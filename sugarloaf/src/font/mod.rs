...
impl FontLibrary {
    ...
    pub fn ensure_static_font(&self, data: &'static [u8]) -> Option<usize> {
        let font_data = FontData::from_static_slice(data).ok()?;
        if let Some(ps_name) = font_data.postscript_name() {
            if let Some(existing) = self.font_id_for_postscript_name(ps_name) {
    ...
    /// Wasm stub: no system font enumeration in the browser, so the
    /// cascade-discovery slow path has nowhere to walk. Mirrors the
    /// macOS / Linux / Windows signatures and returns `None`, which
    /// tells `find_best_font_match` to fall back to glyph-missing
    /// (the same behaviour native gets when no system font covers
    /// the codepoint).
...
impl FontLibraryData {
    ...
    /// Register a bundled static font and return its Sugarloaf `font_id`.
    ///
    /// This is intended for small first-party style packs that ship with the
    /// application. Repeated calls are cheap: loaded faces are deduped by their
    /// PostScript name when the font exposes one.
    pub fn ensure_static_font(&mut self, data: &'static [u8]) -> Option<usize> {
        let font_data = FontData::from_static_slice(data).ok()?;
        if let Some(ps_name) = font_data.postscript_name() {
            if let Some(id) = self.font_id_for_postscript_name(ps_name) {
                return Some(id);
            }
        }
        let id = self.inner.len();
    ...
    pub fn load(&mut self, mut spec: SugarloafFonts) -> Vec<SugarloafFont> {
        ...
        let nerd_symbols_id = self.len();
        self.insert(
            FontData::from_static_slice(constants::FONT_SYMBOLS_NERD_FONT_MONO).unwrap(),
        );
        symbol_maps.extend([
            SymbolMap {
...
/// Wasm stub: `ttf-parser` lives in the non-wasm dep block, and the
/// browser path doesn't do cascade discovery or path-based resolution
/// (it loads font bytes the host hands it), so the PS name lookup is
/// effectively unused. Returning `None` is the same answer the native
