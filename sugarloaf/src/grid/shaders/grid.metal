...
struct Uniforms {
    ...
    float4   clip_rect;        //        80
    float4   cursor_color;     //        96
    float4   cursor_bg_color;  //       112
    float2   cell_size;        //       128
    uint2    grid_size;        //       136
    uint2    cursor_pos;       //       144
    uint2    _pad_cursor;      //       152
    float    min_contrast;     //       160
    uint     flags;            //       164
    uint     padding_extend;   //       168
    uint     input_colorspace; //       172
    float    editor_pixel_offset_y; // 176 — uniform smooth-scroll offset
    // 3 individual uints (NOT uint3 — that would force 16-byte
    // alignment and bloat the struct to 208 bytes, mismatching the
    // 192-byte CPU GridUniforms).
    uint     _pad_editor_0;    //       180
    uint     _pad_editor_1;    //       184
    uint     _pad_editor_2;    //       188
};
...
// Per-cell background. Matches `CellBg` in cell.rs.
struct CellBgData {
    uchar4 rgba;
    int pixel_offset_y;
};
...
fragment float4 grid_bg_fragment(
    ...
    constant CellBgData* cells    [[buffer(1)]]
) {
 ...
 // left (.w) and top (.x) to locate the grid origin.
    ...
    if (uniforms.clip_rect.z > 0.0 && uniforms.clip_rect.w > 0.0) {
        if (in.position.x < uniforms.clip_rect.x ||
            in.position.x >= uniforms.clip_rect.x + uniforms.clip_rect.z ||
            in.position.y < uniforms.clip_rect.y ||
            in.position.y >= uniforms.clip_rect.y + uniforms.clip_rect.w) {
            return bg;
        }
    }
 ...
 // under this fragment, while the grid origin itself stays fixed.
    int idx = grid_pos.y * int(uniforms.grid_size.x) + grid_pos.x;
    ...
    // granularity so text (which rounds the SUM of quad.y + offset
    // in the text vertex shader) stays aligned with the bg cell
    // underneath it during smooth scroll. Rounding the offset alone
    // here made the bg jump in 1-pixel steps at each half-pixel of
    // spring decay, desyncing from the text rounding schedule —
    // text appeared to swim inside its cell. Matches Ghostty.
    float editor_pixel_offset_y = uniforms.editor_pixel_offset_y + cell_pixel_offset_y;
    if (editor_pixel_offset_y != 0.0) {
        float2 adjusted = in.position.xy - float2(0.0, editor_pixel_offset_y);
        int2 shifted = int2(floor((adjusted - uniforms.grid_padding.wx) / uniforms.cell_size));
        if (shifted.x >= 0 && shifted.x < int(uniforms.grid_size.x) &&
            shifted.y >= 0 && shifted.y < int(uniforms.grid_size.y)) {
            grid_pos = shifted;
            idx = grid_pos.y * int(uniforms.grid_size.x) + grid_pos.x;
            cell = cells[idx];
        } else {
            return float4(0.0);
        }
 ...
 // Load the cell and convert to normalized premultiplied color.
    uchar4 rgba = cell.rgba;
    float4 color = float4(rgba) / 255.0;
...
struct CellTextVertexIn {
    ...
    int     pixel_offset_y [[attribute(7)]];
};
...
vertex CellTextVertexOut grid_text_vertex(
 ...
 // inside the drawable — `grid_padding` is (top, right, bottom, left).
    ...
    float glyph_pixel_offset_y = float(in.pixel_offset_y);
    float4 clip_rect = uniforms.clip_rect;
    // Round the COMBINED sum (quad.y + offset + per-cell offset)
    // as a single operation — matches Ghostty's cell_text.v.glsl
    // and the bg shader (which now uses raw float offset for
    // sub-pixel cell shifting). Both paths re-evaluate their
    // integer snap at the SAME half-pixel pivot, so text stays
    // glued to its bg cell during long continuous scroll instead
    // of appearing to swim inside it.
...
fragment float4 grid_text_fragment(
    ...
    if (in.clip_rect.z > 0.0 && in.clip_rect.w > 0.0) {
        ...
        if (px < in.clip_rect.x || px >= in.clip_rect.x + in.clip_rect.z ||
            py < in.clip_rect.y || py >= in.clip_rect.y + in.clip_rect.w) {
            discard_fragment();
        }
    }
