...
/// GPU buffer; the shader reads the cell under each framebuffer pixel.
///
...
pub struct CellBg {
    ...
    pub rgba: [u8; 4],
    /// Rounded vertical pixel offset applied by smooth editor scrolling.
    /// Zero for normal terminal rows and fixed UI rows.
...
///
/// Total size is **36 bytes** — verified by the `size_of` const assert
...
pub struct CellText {
    ...
    pub bools: u8,
    /// Explicit padding to align `pixel_offset_y` to a 4-byte boundary.
    pub _pad: [u8; 2],
    /// Rounded vertical pixel offset for this glyph. Editor buffer
    /// scrolling sets this per glyph so the pane origin can stay fixed.
    pub pixel_offset_y: i32,
}
...
const _: () = {
    ...
    assert!(std::mem::size_of::<CellBg>() == 8);
    assert!(std::mem::size_of::<CellText>() == 36);
};
...
///
/// Current layout: 192 bytes, 16-byte aligned.
#[repr(C)]
...
pub struct GridUniforms {
    ...
    // --- 16-byte block: clip_rect (vec4) at offset 80 ---
    /// Optional drawable-pixel clip rect (x, y, w, h). Zero width/height disables.
    pub clip_rect: [f32; 4],
    // --- 16-byte block: cursor_color (vec4) at offset 96 ---
    ...
    // --- 16-byte block: cursor_bg_color (vec4) at offset 112 ---
    ...
    pub grid_size: [u32; 2],
    // --- 16-byte block of 8-aligned pairs at offset 144 ---
    ...
    // --- 16-byte block of scalars at offset 160 ---
    ...
    // --- 16-byte block of scalars at offset 176 ---
    /// Editor-scroll vertical pixel offset, applied uniformly to every
    /// content cell + glyph in the bg / text passes. Single uniform
    /// instead of per-cell so smooth scroll frames don't have to dirty
    /// the whole bg/fg upload — at 165Hz on a 113×162 grid that
    /// re-upload was costing ~6ms/frame and capping us at ~75 fps.
    /// Zero for terminal panes; set per-frame for editor panes.
    pub editor_pixel_offset_y: f32,
    /// Padding to keep the struct a multiple of 16 bytes (std140 / WGSL).
    pub _pad_editor_offset: [u32; 3],
}
...
const _: () = {
    ...
    assert!(std::mem::size_of::<GridUniforms>() == 192);
};
