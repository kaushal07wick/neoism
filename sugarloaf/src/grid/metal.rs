...
impl MetalGridRenderer {
    ...
    pub fn copy_row(&mut self, src: u32, dst: u32) {
        ...
        let count = cols * self.rows as usize;
        let buf = &self.bg_buffers[0];
        unsafe {
            let ptr = buf.contents() as *mut CellBg;
            let cells = std::slice::from_raw_parts_mut(ptr, count);
            cells.copy_within(src_start..src_start + cols, dst_start);
        }
    }
    ...
    pub fn snapshot_row_rect(
        ...
        let count = cols_total * self.rows as usize;
        let bg = unsafe {
            let ptr = self.bg_buffers[0].contents() as *const CellBg;
            let cells = std::slice::from_raw_parts(ptr, count);
            cells[bg_start..bg_end].to_vec()
        };
        let fg = self
            .fg_rows
            .get(row as usize + 1)
    ...
    pub fn write_row_snapshot(&mut self, row: u32, snapshot: &GridRowSnapshot) {
        ...
        let count = cols_total * self.rows as usize;
        unsafe {
            let ptr = self.bg_buffers[0].contents() as *mut CellBg;
            let cells = std::slice::from_raw_parts_mut(ptr, count);
            cells[bg_start..bg_end]
    ...
    pub fn set_pixel_offset_y_for_rows(
        ...
        unsafe {
            let ptr = buf.contents() as *mut CellBg;
            let cells = std::slice::from_raw_parts_mut(ptr, cols * self.rows as usize);
            for row in start..end {
                let row_start = row * cols;
                for cell in &mut cells[row_start..row_start + cols] {
...
fn build_text_pipeline(device: &Device) -> RenderPipelineState {
    ...
    a.set_buffer_index(0);
    a.set_offset(29);
    // attribute 7: pixel_offset_y: i32 @ offset 32 → Int
    let a = attrs.object_at(7).unwrap();
    a.set_format(MTLVertexFormat::Int);
    a.set_buffer_index(0);
    a.set_offset(32);
    // Layout: per-instance, stride = sizeof(CellText) = 36.
