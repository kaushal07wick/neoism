...
//! call sites in `neoism::renderer` land in Phase 2.
...
impl GridRenderer {
    ...
    /// Copy one content row to another while retargeting foreground
    /// glyph instances to the destination row. This is used by smooth
    /// editor scrolling when the integer source row changes by a small
    /// amount: most resident rows can shift in-place and only newly
    /// exposed rows need CPU shaping/raster emission.
    pub fn copy_row(&mut self, src: u32, dst: u32) {
        match self {
            #[cfg(target_os = "macos")]
            GridRenderer::Metal(r) => r.copy_row(src, dst),
            #[cfg(feature = "wgpu")]
            GridRenderer::Wgpu(r) => r.copy_row(src, dst),
            #[cfg(target_os = "linux")]
            GridRenderer::Vulkan(r) => r.copy_row(src, dst),
            #[cfg(not(target_arch = "wasm32"))]
            GridRenderer::Cpu(r) => r.copy_row(src, dst),
        }
    }
    ...
    pub fn snapshot_row_rect(
        ...
        match self {
            #[cfg(target_os = "macos")]
            GridRenderer::Metal(r) => r.snapshot_row_rect(row, cols),
            #[cfg(feature = "wgpu")]
            GridRenderer::Wgpu(r) => r.snapshot_row_rect(row, cols),
            #[cfg(target_os = "linux")]
            GridRenderer::Vulkan(r) => r.snapshot_row_rect(row, cols),
            #[cfg(not(target_arch = "wasm32"))]
            GridRenderer::Cpu(r) => r.snapshot_row_rect(row, cols),
        }
    }
    ...
    pub fn write_row_snapshot(&mut self, row: u32, snapshot: &GridRowSnapshot) {
        match self {
            #[cfg(target_os = "macos")]
            GridRenderer::Metal(r) => r.write_row_snapshot(row, snapshot),
            #[cfg(feature = "wgpu")]
            GridRenderer::Wgpu(r) => r.write_row_snapshot(row, snapshot),
            #[cfg(target_os = "linux")]
            GridRenderer::Vulkan(r) => r.write_row_snapshot(row, snapshot),
            #[cfg(not(target_arch = "wasm32"))]
            GridRenderer::Cpu(r) => r.write_row_snapshot(row, snapshot),
        }
    }

    /// Clear smooth-scroll pixel offsets from content rows. Cursor
    /// sprite slots are intentionally left alone; they are rewritten by
    /// `set_block_cursor` / `set_non_block_cursor` every frame.
    pub fn clear_pixel_offsets(&mut self) {
        match self {
            #[cfg(target_os = "macos")]
            GridRenderer::Metal(r) => r.clear_pixel_offsets(),
            #[cfg(feature = "wgpu")]
            GridRenderer::Wgpu(r) => r.clear_pixel_offsets(),
            #[cfg(target_os = "linux")]
            GridRenderer::Vulkan(r) => r.clear_pixel_offsets(),
            #[cfg(not(target_arch = "wasm32"))]
            GridRenderer::Cpu(r) => r.clear_pixel_offsets(),
        }
    }

    /// Apply a rounded vertical pixel offset to a range of content rows.
    /// Used by editor buffer scrolling so only live/snapshot buffer rows
    /// slide; inactive buffer slots stay fixed/transparent.
    pub fn set_pixel_offset_y_for_rows(
        ...
        match self {
            #[cfg(target_os = "macos")]
            GridRenderer::Metal(r) => r.set_pixel_offset_y_for_rows(rows, pixel_offset_y),
            #[cfg(feature = "wgpu")]
            GridRenderer::Wgpu(r) => r.set_pixel_offset_y_for_rows(rows, pixel_offset_y),
            #[cfg(target_os = "linux")]
            GridRenderer::Vulkan(r) => {
                r.set_pixel_offset_y_for_rows(rows, pixel_offset_y)
            }
            #[cfg(not(target_arch = "wasm32"))]
            GridRenderer::Cpu(r) => r.set_pixel_offset_y_for_rows(rows, pixel_offset_y),
        }
    }
