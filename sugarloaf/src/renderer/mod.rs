// Cfg-gated paths produce unreachable / irrefutable warnings when the
// `wgpu` feature isn't enabled. Silence at module scope so the warnings
// don't drown the build output for the default (non-wgpu) configuration.
#![allow(unreachable_code, unreachable_patterns, irrefutable_let_patterns)]
...
pub struct WgpuRenderer {
    ...
    supported_overlay_vertex_buffer: usize,
    supported_overlay_instance_buffer: usize,
...
impl Renderer {
    ...
    pub(crate) fn discard_frame_batches(&mut self) {
        ...
        self.overlay_comp.batches.reset();
    }
    ...
    #[cfg(target_os = "linux")]
    pub(crate) fn set_shader_overlay_vulkan(
        &mut self,
        ...
        if let RendererType::Vulkan(vulkan) = &mut self.brush_type {
            vulkan.set_shader_overlay(config, extent, color_format)?;
        }
        Ok(())
    }

    #[cfg(target_os = "linux")]
    pub(crate) fn has_shader_overlay_vulkan(&self) -> bool {
        matches!(
            &self.brush_type,
            RendererType::Vulkan(vulkan) if vulkan.has_shader_overlay()
        )
    }

    #[cfg(target_os = "linux")]
    pub(crate) fn shader_overlay_scene_view_vulkan(&self) -> Option<ash::vk::ImageView> {
        match &self.brush_type {
            RendererType::Vulkan(vulkan) => vulkan.shader_overlay_scene_view(),
            _ => None,
        }
    }

    #[cfg(target_os = "linux")]
    pub(crate) fn shader_overlay_scene_image_vulkan(&self) -> Option<ash::vk::Image> {
        match &self.brush_type {
            RendererType::Vulkan(vulkan) => vulkan.shader_overlay_scene_image(),
            _ => None,
        }
    }

    #[cfg(target_os = "linux")]
    pub(crate) fn resize_shader_overlay_vulkan(
        &mut self,
        extent: ash::vk::Extent2D,
        color_format: ash::vk::Format,
    ) {
        if let RendererType::Vulkan(vulkan) = &mut self.brush_type {
            vulkan.resize_shader_overlay(extent, color_format);
        }
    }

    #[cfg(target_os = "linux")]
    pub(crate) fn render_shader_overlay_vulkan(
        &mut self,
        cmd: ash::vk::CommandBuffer,
        slot: usize,
        swapchain_image: ash::vk::Image,
        swapchain_view: ash::vk::ImageView,
    ) {
        if let RendererType::Vulkan(vulkan) = &mut self.brush_type {
            vulkan.render_shader_overlay(cmd, slot, swapchain_image, swapchain_view);
        }
    }
    ...
    /// Add a rectangle to the late overlay pass. Late overlays render
    /// after the normal UI text pass, and before `Sugarloaf`'s late
    /// overlay text pass. Use this for popover/menu backplates that must
    /// occlude tab labels, breadcrumbs, and other already-recorded text.
    #[inline]
    #[allow(clippy::too_many_arguments)]
    pub fn overlay_rect(
        &mut self,
        ...
        self.overlay_comp.batches.rect(
            &Rect {
                x,
                y,
                width,
    ...
    /// Add a rounded rectangle to the late overlay pass.
    #[inline]
    #[allow(clippy::too_many_arguments)]
    pub fn overlay_rounded_rect(
        ...
        self.overlay_comp.batches.rounded_rect(
            &Rect {
                x,
                y,
                width,
    ...
    /// Add a quad with per-corner radii to the late overlay pass.
    #[inline]
    #[allow(clippy::too_many_arguments)]
    pub fn overlay_quad(
        &mut self,
        ...
        self.overlay_comp.batches.quad(
            &Rect {
                x,
                y,
                width,
    ...
    pub fn render<'pass>(
        ...
        text: &'pass mut crate::text::Text,
        overlay_text: &'pass mut crate::text::Text,
    ) {
        ...
        if let RendererType::Wgpu(brush) = brush_type {
            ...
            #[cfg(not(target_os = "macos"))]
            {
                text.init_wgpu(&ctx.device, &ctx.queue, ctx.format);
                text.render_wgpu(rpass, [ctx.size.width, ctx.size.height]);

                if !overlay_instances.is_empty() {
                    if overlay_instances.len() > brush.supported_overlay_instance_buffer {
                        brush.overlay_instance_buffer.destroy();
                        brush.supported_overlay_instance_buffer =
                            (overlay_instances.len() as f32 * 1.25) as usize;
                        brush.overlay_instance_buffer =
                            ctx.device.create_buffer(&wgpu::BufferDescriptor {
                                label: Some(
                                    "rich_text::Overlay Instance Buffer (resized)",
                                ),
                                size: mem::size_of::<batch::QuadInstance>() as u64
                                    * brush.supported_overlay_instance_buffer as u64,
                                usage: wgpu::BufferUsages::VERTEX
                                    | wgpu::BufferUsages::COPY_DST,
                                mapped_at_creation: false,
                            });
                ...
                if !overlay_vertices.is_empty() {
                    if overlay_vertices.len() > brush.supported_overlay_vertex_buffer {
                        brush.overlay_vertex_buffer.destroy();
                        brush.supported_overlay_vertex_buffer =
                            (overlay_vertices.len() as f32 * 1.25) as usize;
                        brush.overlay_vertex_buffer =
                            ctx.device.create_buffer(&wgpu::BufferDescriptor {
                                label: Some(
                                    "rich_text::Overlay Vertices Buffer (resized)",
                                ),
                                size: mem::size_of::<Vertex>() as u64
                                    * brush.supported_overlay_vertex_buffer as u64,
                                usage: wgpu::BufferUsages::VERTEX
                                    | wgpu::BufferUsages::COPY_DST,
                                mapped_at_creation: false,
                            });
                ...
                for cmd in overlay_draw_cmds {
                    ...
                    match cmd {
                        batch::DrawCmd::Instanced { offset, count, .. } => {
                            ...
                            rpass.set_vertex_buffer(
                                0,
                                brush.overlay_instance_buffer.slice(byte_offset..),
                            );
                            rpass.draw(0..4, 0..*count);
                        }
                        batch::DrawCmd::Vertices { offset, count, .. } => {
                            if !pipeline_set || current_pipeline_instanced {
                                ...
                                rpass.set_vertex_buffer(
                                    0,
                                    brush.overlay_vertex_buffer.slice(..),
                                );
                                current_pipeline_instanced = false;
                                pipeline_set = true;
                            }
    ...
    pub fn render_overlay<'pass>(
        ...
        let color_views = images.get_texture_views();
        let Some(default_color_view) = color_views.first() else {
            return;
        };
        let mask_texture_view = images.get_mask_texture_view();
        ...
        if !overlay_instances.is_empty() {
            if overlay_instances.len() > brush.supported_overlay_instance_buffer {
                ...
                brush.overlay_instance_buffer =
                    ctx.device.create_buffer(&wgpu::BufferDescriptor {
                        label: Some("rich_text::Overlay Instance Buffer (resized)"),
                        size: mem::size_of::<batch::QuadInstance>() as u64
                            * brush.supported_overlay_instance_buffer as u64,
        ...
        if !overlay_vertices.is_empty() {
            if overlay_vertices.len() > brush.supported_overlay_vertex_buffer {
                ...
                brush.overlay_vertex_buffer =
                    ctx.device.create_buffer(&wgpu::BufferDescriptor {
                        label: Some("rich_text::Overlay Vertices Buffer (resized)"),
                        size: mem::size_of::<Vertex>() as u64
                            * brush.supported_overlay_vertex_buffer as u64,
                        usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
                        mapped_at_creation: false,
        ...
        for cmd in overlay_draw_cmds {
            ...
            let color_view = if color_layer > 0 {
                let idx = (color_layer - 1) as usize;
                color_views.get(idx).unwrap_or(default_color_view)
            } else {
                default_color_view
            };
            let final_mask_view = if mask_layer > 0 {
                mask_texture_view.unwrap_or(default_color_view)
            } else {
                default_color_view
            };
            brush.update_bind_group(ctx, color_view, final_mask_view);
            ...
            match cmd {
                ...
                batch::DrawCmd::Vertices { offset, count, .. } => {
                    if !pipeline_set || current_pipeline_instanced {
                        rpass.set_pipeline(&brush.pipeline);
                        rpass.set_bind_group(0, &brush.constant_bind_group, &[]);
                        rpass.set_vertex_buffer(0, brush.overlay_vertex_buffer.slice(..));
                        current_pipeline_instanced = false;
                        pipeline_set = true;
                    }
                    rpass.set_bind_group(1, &brush.layout_bind_group, &[]);
    ...
    pub fn render_metal(
        ...
        overlay_text: &mut crate::text::Text,
    ) {
    ...
    pub fn render_vulkan(
        ...
        if let RendererType::Vulkan(brush) = &mut self.brush_type {
            ...
            // Below + above passes share one per-slot instance
            // buffer. Pass `start_index = 0` for the first call,
            // then `below.len()` for the second so they don't
            // overlap. Without this, the AboveText pass clobbers
            // the BelowText pass's instance data and earlier
            // draws sample whatever the later pass wrote in the
            // same slot — surfaced as splash letter `n` taking
            // the position of an agent-icon overlay when both
            // were active in the same frame.
            brush.render_image_overlays(cmd, slot, viewport, &below, 0);
            brush.render_quads(cmd, slot, viewport, &self.instances);
            brush.render_geometry(cmd, slot, viewport, &self.vertices);
            brush.render_image_overlays(cmd, slot, viewport, &above, below.len());
...
impl WgpuRenderer {
    pub fn new(context: &WgpuContext) -> Self {
        ...
        let supported_overlay_vertex_buffer = supported_vertex_buffer;
        let overlay_vertex_buffer =
            context.device.create_buffer(&wgpu::BufferDescriptor {
                label: Some("rich_text::Overlay Vertices Buffer"),
                size: mem::size_of::<Vertex>() as u64
                    * supported_overlay_vertex_buffer as u64,
                usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
                mapped_at_creation: false,
            });

        let supported_overlay_instance_buffer = supported_instance_buffer;
        let overlay_instance_buffer =
            context.device.create_buffer(&wgpu::BufferDescriptor {
                label: Some("rich_text::Overlay Instance Buffer"),
                size: mem::size_of::<batch::QuadInstance>() as u64
                    * supported_overlay_instance_buffer as u64,
                usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
                mapped_at_creation: false,
            });
        ...
        WgpuRenderer {
            ...
            supported_overlay_vertex_buffer,
            supported_overlay_instance_buffer,
            current_transform,
            image_pipeline,
