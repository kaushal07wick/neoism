...
fn vs_main(
    ...
    // `source_rect` is `[u0, v0, u1, v1]` (origin, end) — matches the
    // Vulkan GLSL + Metal pipelines so the wgpu backend (WebGPU/WebGL2)
    // samples each instance's slice the same way the native backends do.
