...
pub fn render_cpu(
    ...
    let overlay_vertices = renderer.overlay_vertices();
    let overlay_quad_instances = renderer.overlay_instances();
    let text_instances = text.instances();
    let overlay_text_instances = overlay_text.instances();
    ...
    let frame_hash = {
        ...
        h.write(inst_bytes);
        let overlay_inst_bytes: &[u8] = bytemuck::cast_slice(overlay_quad_instances);
        h.write(overlay_inst_bytes);
        let overlay_vert_bytes: &[u8] = bytemuck::cast_slice(overlay_vertices);
        h.write(overlay_vert_bytes);
        ...
        h.write_usize(overlay_text_instances.len());
        if !overlay_text_instances.is_empty() {
            let bytes = unsafe {
                std::slice::from_raw_parts(
                    overlay_text_instances.as_ptr() as *const u8,
                    std::mem::size_of_val(overlay_text_instances),
                )
            };
            h.write(bytes);
        }
    ...
    if !overlay_quad_instances.is_empty() {
        let buf_slice: &mut [u32] = &mut buffer;
        for inst in overlay_quad_instances {
            if inst.layers[0] != 0 || inst.layers[1] != 0 || inst.underline_style > 1 {
                continue;
            }
            draw_quad_instance(buf_slice, buf_w, buf_h, inst);
        }
    ...
    if !overlay_vertices.is_empty() {
        let buf_slice: &mut [u32] = &mut buffer;
        let mut i = 0usize;
        while i + 5 < overlay_vertices.len() {
            let chunk = &overlay_vertices[i..i + 6];
            i += 6;
            let q = parse_quad(chunk);
            if q.max_x - q.min_x <= 0.0 || q.max_y - q.min_y <= 0.0 {
                continue;
            ...
            if q.mask_layer > 0 || q.color_layer > 0 {
                continue;
            }
            let r = (q.color[0].clamp(0.0, 1.0) * 255.0) as u8;
            let g = (q.color[1].clamp(0.0, 1.0) * 255.0) as u8;
            ...
            if a == 255 {
                let fill = PendingFill {
                    ...
                    x1,
                    y1,
                    packed: pack_opaque(r, g, b),
                };
                flush_fill(buf_slice, buf_w, &fill);
            } else if a > 0 {
                fill_translucent_simd(buf_slice, buf_w, x0, y0, x1, y1, r, g, b, a);
            }
        }
    ...
    overlay_text.render_cpu(&mut buffer, ctx.width_px, ctx.height_px);
