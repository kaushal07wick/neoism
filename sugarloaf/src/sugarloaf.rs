...
pub struct Sugarloaf<'a> {
    ...
    #[cfg(feature = "wgpu")]
    shader_overlay_brush: Option<ShaderOverlayBrush>,
...
impl Default for Colorspace {
    fn default() -> Colorspace {
        // See `neoism-backend::config::window::Colorspace::default` — the
...
impl Default for SugarloafRenderer {
    fn default() -> SugarloafRenderer {
        ...
        // Request WebGL directly on wasm. In wgpu 28, requesting
        // BROWSER_WEBGPU when `navigator.gpu` exists selects the WebGPU
        // instance path even if `requestAdapter()` later returns null,
        // and that path never falls through to WebGL.
        let default_backend = SugarloafBackend::Wgpu(wgpu::Backends::GL);

        // wasm32 without the `wgpu` feature has no usable backend (Cpu
        // owns softbuffer, which requires a real window handle and is
        // gated off for wasm). Building wasm32 without `web` / `wgpu`
        // is not a supported configuration — fail loudly so the caller
        // adds `--features web`.
        #[cfg(all(target_arch = "wasm32", not(feature = "wgpu")))]
        compile_error!(
            "sugarloaf on wasm32 requires the `web` feature (which pulls in `wgpu`). \
             Rebuild with `--features web`."
        );
...
impl Sugarloaf<'_> {
    /// Synchronous constructor. Available on every native target.
    ///
    /// Not available on `wasm32-unknown-unknown`: the wgpu adapter /
    /// device requests inside `WgpuContext::new` resolve via
    /// `futures::executor::block_on`, which panics under
    /// `wasm-bindgen`'s single-threaded event loop. Browser callers
    /// must use `Sugarloaf::from_canvas` (async) instead.
    #[cfg(not(target_arch = "wasm32"))]
    pub fn new<'a>(
        ...
        let mut overlay_text = crate::text::Text::new(font_library);
        overlay_text.set_scale_factor(state.style.scale_factor);
        if matches!(ctx.inner, crate::context::ContextType::Cpu(_)) {
            text.init_cpu();
            overlay_text.init_cpu();
        }
    ...
    /// Async constructor for browser targets. Creates a wgpu surface
    /// from `canvas` and awaits adapter / device requests inside
    /// `WgpuContext::new_async`. The shape mirrors `new` but skips the
    /// `SugarloafWindow` (which carries a `RawWindowHandle` meaningless
    /// to a canvas) and the CPU-backend init (no softbuffer on wasm).
    ///
    /// Errors propagate as a plain `SugarloafErrors` with an empty
    /// `fonts_not_found` and the failure reason inside (browser
    /// adapter / device request failure, no compatible surface, etc.).
    /// Once we surface a typed error we'll add it; for now the message
    ...
    pub async fn from_canvas<'a>(
        ...
        font_library: &FontLibrary,
        layout: RootStyle,
    ) -> Result<Sugarloaf<'a>, String> {
        let font_features = renderer.font_features.to_owned();
        let colorspace = renderer.colorspace;
        let ctx = match Context::from_canvas(canvas, size, scale, renderer).await {
            Ok(ctx) => ctx,
            Err(e) => {
                tracing::error!("Sugarloaf::from_canvas: {e}");
                return Err(e);
            }
        };
    ...
    /// Placeholder helper used by `from_canvas` to fabricate a
    /// `Sugarloaf` instance when surface creation has failed. The
    /// caller will discard the wrapper — but Rust's error type
    /// (`SugarloafWithErrors`) still expects an owned instance. This
    /// is the wasm32 equivalent of native's "return early before
    /// wrapping" pattern; the field values are inert (no GPU state
    /// constructed beyond the empty Renderer + Context).
    #[cfg(all(target_arch = "wasm32", feature = "wgpu"))]
    fn placeholder_after_err<'a>(
        _font_library: &FontLibrary,
        _layout: RootStyle,
    ) -> Sugarloaf<'a> {
        // We can't construct a Sugarloaf without a Context, and a
        // Context requires either a window handle or a canvas. The
        // honest approach is to make the placeholder unreachable at
        // runtime: callers MUST treat the Err arm as fatal and never
        // unwrap into `instance`. Panic loudly if anyone does — better
        // than a silent half-initialized state.
        unreachable!(
            "Sugarloaf::from_canvas placeholder reached — surface creation \
             returned Err and the caller tried to access `instance`. This \
             field is reserved for native's font-fallback path; on wasm \
             you must abort on Err."
        );
    }
    ...
    #[inline]
    pub fn uses_native_vulkan(&self) -> bool {
        #[cfg(target_os = "linux")]
        {
            matches!(self.ctx.inner, crate::context::ContextType::Vulkan(_))
        }
    ...
    #[inline]
    pub fn ensure_static_font(&mut self, data: &'static [u8]) -> Option<usize> {
        self.state
            .content
            .font_library()
            .inner
            .write()
            .ensure_static_font(data)
    }
    ...
    /// requests entirely. Gated off on wasm32 because
    /// `components::filters::builtin` reaches for `std::fs` to stage
    /// the bundled CRT presets.
    #[cfg(all(feature = "wgpu", not(target_arch = "wasm32")))]
    ...
    pub fn set_shader_overlay(
        ...
        match &mut self.ctx.inner {
            ...
            crate::context::ContextType::Wgpu(ctx) => {
                self.shader_overlay_brush = ShaderOverlayBrush::load(ctx, config)?;
            }
            #[cfg(target_os = "linux")]
            crate::context::ContextType::Vulkan(ctx) => {
                self.renderer.set_shader_overlay_vulkan(
                    &config,
                    ash::vk::Extent2D {
            ...
            crate::context::ContextType::_Phantom(_) => {
                if !config.is_empty() {
                    return Err(ShaderOverlayError::UnsupportedBackend {
    ...
    ///
    /// On `wasm32-unknown-unknown` the `image_rs::open(path)` call is
    /// not available (no `std::fs`); the function logs a warning and
    /// returns Err so callers can swap to a byte-driven entrypoint
    /// once we have one.
    #[inline]
    pub fn set_background_image(
        ...
        {
            tracing::debug!(
                "set_background_image: path-based loading not supported on wasm32 \
                 (no std::fs); request for '{}' ignored",
                image.path
            );
            let _ = image;
            return Err(
                "background image path loading is unavailable on wasm32".to_string()
            );
        }
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            if img_w == 0 || img_h == 0 {
                let msg =
                    format!("'{}' decoded to a {}x{} image", image.path, img_w, img_h);
    ...
    /// Draw a triangle in a specific compositor order.
    /// Coordinates are in logical pixels (scaled internally).
    #[inline]
    #[allow(clippy::too_many_arguments)]
    pub fn triangle_ordered(
        &mut self,
        x1: f32,
        y1: f32,
        x2: f32,
    ...
    /// above late overlay quads. Pair with `overlay_rect` /
    /// `overlay_rounded_rect` for menus, popovers, and tooltips that
    /// must cover previously drawn UI text.
    #[inline]
    pub fn overlay_text_mut(&mut self) -> &mut crate::text::Text {
        &mut self.overlay_text
    }

    /// Add a rectangle to Sugarloaf's late overlay pass. It renders
    /// after normal UI text and before `overlay_text_mut()` labels.
    #[inline]
    #[allow(clippy::too_many_arguments)]
    pub fn overlay_rect(
        &mut self,
    ...
    /// Add a rounded rectangle to Sugarloaf's late overlay pass.
    #[inline]
    #[allow(clippy::too_many_arguments)]
    ...
    /// Add a quad with per-corner radii to Sugarloaf's late overlay pass.
    #[inline]
    #[allow(clippy::too_many_arguments)]
    pub fn overlay_quad(
        &mut self,
    ...
    pub fn rescale(&mut self, scale: f32) {
        ...
        self.overlay_text.set_scale_factor(scale);
    }
    ...
    /// sites in neoism build the slice with one entry per panel.
    #[inline]
    ...
    pub fn render_vulkan(
        ...
        self.overlay_text.init_vulkan(ctx);
        for (grid, _) in grids.iter_mut() {
            grid.prepare_vulkan(ctx, cmd, frame.slot);
        }
        self.text.prepare_vulkan(ctx, cmd, frame.slot);
        self.overlay_text.prepare_vulkan(ctx, cmd, frame.slot);

        self.renderer
            .resize_shader_overlay_vulkan(frame.extent, frame.format);
        let shader_overlay_active = self.renderer.has_shader_overlay_vulkan();

        let scene_target = if shader_overlay_active {
            self.renderer
                .shader_overlay_scene_view_vulkan()
                .zip(self.renderer.shader_overlay_scene_image_vulkan())
        } else {
            None
        };
        if let Some((_, scene_image)) = scene_target {
            vkr::transition_overlay_image(
                &device,
                cmd,
                scene_image,
        ...
        let color_attachment = if let Some((scene_view, _)) = scene_target {
            ash::vk::RenderingAttachmentInfo::default()
                ...
                .clear_value(ash::vk::ClearValue {
                    color: ash::vk::ClearColorValue { float32: bg },
                })
        } else {
            vkr::build_color_attachment(&frame, bg)
        };
        ...
        self.overlay_text.render_vulkan(
            cmd,
            frame.slot,
            [frame.extent.width as f32, frame.extent.height as f32],
        );
        ...
        if scene_target.is_some() {
            self.renderer.render_shader_overlay_vulkan(
                cmd,
                frame.slot,
                frame.image,
                frame.image_view,
    ...
    pub fn render_wgpu(
        ...
        match ctx.surface.get_current_texture() {
            Ok(frame) => {
                ...
                let overlay_target = self.shader_overlay_brush.as_ref().map(|_| {
                    ctx.device.create_texture(&wgpu::TextureDescriptor {
                        label: Some("Shader Overlay Frame Input"),
                        size: wgpu::Extent3d {
                            width: ctx.size.width.max(1.0) as u32,
                            height: ctx.size.height.max(1.0) as u32,
                ...
                let render_texture = overlay_target.as_ref().unwrap_or(&frame.texture);
                let view =
                ...
                // librashader filter chain isn't compiled on wasm32 —
                // see `components::mod.rs`. Skip the post-process pass
                // entirely on the browser target.
                #[cfg(not(target_arch = "wasm32"))]
                ...
                if let (Some(ref mut shader_overlay_brush), Some(overlay_target)) =
                    (&mut self.shader_overlay_brush, overlay_target.as_ref())
                {
                    shader_overlay_brush.render(
                        ctx,
                        &mut encoder,
                        overlay_target,
                        &frame.texture,
            ...
            Err(error) => match error {
                ...
                wgpu::SurfaceError::Lost => {
                    tracing::warn!(
                        ...
                        "wgpu surface lost; reconfiguring before next frame"
                    );
                ...
                wgpu::SurfaceError::Outdated | wgpu::SurfaceError::Timeout => {
                    tracing::debug!(
                        target: "sugarloaf::wgpu",
                        ?error,
                        "skipping wgpu frame after transient surface acquire error"
                    );
                }
                error => {
                    tracing::warn!(
                        target: "sugarloaf::wgpu",
                        ?error,
                        "skipping wgpu frame after surface acquire error"
                    );
                }
            },
        }
