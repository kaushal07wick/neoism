...
pub trait Handler {
    ...
    /// OSC 133 semantic prompt markers. These mark shell prompt,
    /// command, and completion boundaries without printing cells.
    fn semantic_prompt(&mut self, _: SemanticPromptAction) {}
    ...
    fn set_progress_report(&mut self, _: crate::ProgressReport) {}
...
impl<U: Handler, T: Timeout> copa::Perform for Performer<'_, U, T> {
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            b"7" => {
                let Some(raw_path) = params.get(1) else {
                    unhandled(params);
                    return;
                };
            ...
            // FinalTerm/OSC 133 semantic prompt markers. We only need the
            // lifecycle actions for app-owned input/block layout; options
            // such as aid/redraw/k are ignored for now.
            b"133" => {
                if params.len() < 2 {
                    unhandled(params);
                    return;
                ...
                let action = match params[1] {
                    b"A" => Some(SemanticPromptAction::PromptStart),
                    b"B" => Some(SemanticPromptAction::PromptEnd),
                    b"C" => Some(SemanticPromptAction::CommandStart),
                    b"D" => {
                        let exit = params.get(2).and_then(|raw| {
                            simd_utf8::from_utf8_fast(raw).ok().and_then(|s| {
                        ...
                        Some(SemanticPromptAction::CommandFinished(exit))
                    }
                    _ => None,
                };

                if let Some(action) = action {
                    self.handler.semantic_prompt(action);
                    return;
                }
            ...
            b"9" => {
                ...
                if params.len() >= 3 && params[1] == b"4" {
                    ...
                    let state = match params[2] {
                        b"0" => Some(crate::ProgressState::Remove),
                        b"1" => Some(crate::ProgressState::Set),
                        b"2" => Some(crate::ProgressState::Error),
                        b"3" => Some(crate::ProgressState::Indeterminate),
                        b"4" => Some(crate::ProgressState::Pause),
                        _ => None,
                    };
                    ...
                    if let Some(state) = state {
                        ...
                        let report = crate::ProgressReport { state, progress };
            ...
            b"777" => {
                ...
                if params.len() >= 3 && params[1] == b"neoism" {
                    match params[2] {
                        b"new" => {
                            self.handler.open_editor_tab(None);
                            return;
                        }
                        b"open" if params.len() >= 4 => {
