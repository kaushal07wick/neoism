...
use crate::colors::{self, term::TermColors, ColorRgb};
...
use crate::graphics::{GraphicData, MAX_GRAPHIC_DIMENSIONS};
use crate::handler::{Handler, SemanticPromptAction};
...
pub use crate::damage::LineDamage;
...
pub struct ShellPromptState {
    /// True after OSC 133;B until OSC 133;C. This is the safe window
    /// where the app may own editable command input.
...
// `LineDamage` was moved into `crate::damage` in phase 3b; it is
// re-exported above as `pub use crate::damage::LineDamage;` for call
// sites that still resolve through `crosswords::LineDamage`.
...
fn vs_is_valid_base(base: char, vs: char) -> bool {
    use neoism_grapheme_width::emoji::Presentation;
    let mut buf = [0u8; 8];
...
pub struct Crosswords {
    ...
    /// Config-seeded blink default, restored by DECSCUSR 0 ("reset to
    /// default") — without this, any TUI resetting the cursor on exit
    /// (nvim does) permanently disabled config-enabled blinking.
    pub default_blinking_cursor: bool,
    /// Stable terminal identifier the host uses to associate this
    /// terminal with its route / tab / session. Replaces the historical
    /// `route_id: usize` field — kept as a typed value so the native
    /// adapter can map it back to a `route_id` at drain time.
    pub terminal_id: TerminalId,
    title_stack: Vec<String>,
    pub current_directory: Option<std::path::PathBuf>,
    shell_prompt: ShellPromptState,
    ...
    /// Per-pane "scrolled-away" line snapshots — populated only on
    /// editor (nvim) panes via `nvim_events.rs::RedrawEvent::Scroll`,
    /// BEFORE the in-place cell shift overwrites the rows. The
    /// renderer reads these and writes them into the GPU grid's
    /// "above-visible" buffer slots so the scroll animation has real
    /// content to slide instead of revealing a blank band. Front of
    /// the deque is most-recent (the row that just scrolled away);
    /// older entries decay further into the past. Cap is the same as
    /// `EDITOR_SCROLL_BUFFER_ROWS` on the renderer side — older
    /// entries are dropped when the deque is full. Empty for
    /// terminal panes.
    pub editor_scroll_snapshot_above:
        std::collections::VecDeque<crate::crosswords::grid::row::Row<Square>>,
    /// Symmetric snapshot for scroll-UP direction (k / page-up / Ctrl-U).
    /// Captured by the negative-rows branch in `nvim_events.rs` BEFORE
    /// the in-place shift overwrites the bottom rows. push_back
    /// bottom-up: deque[0] = oldest bottom row scrolled away,
    /// deque[len-1] = most recent (just below current viewport bottom
    /// after the scroll). Renderer mirrors the above-buffer logic onto
    /// the BELOW grid slots so scrolling up shows the just-revealed
    /// content sliding into view from below.
    pub editor_scroll_snapshot_below:
        std::collections::VecDeque<crate::crosswords::grid::row::Row<Square>>,
    /// Neovide-style editor scrollback ring. For embedded nvim panes,
    /// smooth scrolling samples from this 2x-viewport buffer during the
    /// scroll spring instead of mixing current rows with narrow
    /// above/below snapshots. Index `0..visible_rows` is always the
    /// latest viewport after the most recent redraw flush; signed
    /// negative/overflow indices wrap through the second half to expose
    /// rows that just scrolled away.
    pub editor_scrollback_lines: Vec<Option<crate::crosswords::grid::row::Row<Square>>>,
    editor_scrollback_current_index: isize,
    editor_scrollback_visible_rows: usize,
    editor_scrollback_columns: usize,
    ...
    /// Phase 2 effect buffer.
    ///
    /// All terminal-originated side effects (`Bell`, `PtyWrite`,
    /// clipboard requests, render requests, exit, …) are pushed here
    /// rather than being dispatched into a native event proxy. Phase
    /// 3a removed the historical `event_proxy: U` field and the
    /// `EventListener` generic — `Crosswords` is now a plain,
    /// non-generic terminal state machine with no native-side
    /// dependencies. The host drains this buffer right after each
    /// `Parser::advance` call (see `performer::Machine::pty_read` and
    /// `effects_adapter::dispatch_terminal_effects`) and translates
    /// the effects into whatever the host actually does — `RioEvent`s
    /// in the native frontend, wire messages in the daemon, JS
    /// callbacks in the wasm host.
    effects: Vec<crate::TerminalEffect>,
}
...
impl Crosswords {
    pub fn new<D: Dimensions>(
        ...
        Crosswords {
            ...
            default_blinking_cursor: false,
            terminal_id,
            title_stack: Default::default(),
            current_directory: None,
            shell_prompt: ShellPromptState::default(),
            damage_event_in_flight: false,
            editor_scroll_snapshot_above: std::collections::VecDeque::new(),
            editor_scroll_snapshot_below: std::collections::VecDeque::new(),
            editor_scrollback_lines: Vec::new(),
            editor_scrollback_current_index: 0,
            editor_scrollback_visible_rows: 0,
            editor_scrollback_columns: 0,
    ...
    /// Push a terminal-originated side effect into the per-terminal
    /// effect buffer. Drained by the host after each parser advance
    /// (see `effects_adapter::dispatch_terminal_effects`).
    pub(crate) fn push_effect(&mut self, effect: crate::TerminalEffect) {
    ...
    pub fn mark_fully_damaged(&mut self) {
        // Only emit effect if we weren't already fully damaged
        let was_damaged = self.damage.full;
        self.damage.full = true;

        // Request a render to display the damage.
        if !was_damaged {
            self.push_effect(crate::TerminalEffect::RenderRequest);
        }
    }
    ...
    #[inline]
    pub fn shell_prompt_state(&self) -> ShellPromptState {
        self.shell_prompt
    }
    ...
    pub fn scroll_display(&mut self, scroll: Scroll) {
        ...
        self.push_effect(crate::TerminalEffect::MouseCursorDirty);
    ...
    pub fn send_graphics_updates(&mut self) {
        if self.graphics.has_pending_updates() {
            if let Some(queues) = self.graphics.take_queues() {
                self.push_effect(crate::TerminalEffect::GraphicsUpdate(
                    crate::GraphicsUpdate::new(queues),
                ));
            }
        }
    }
    ...
    pub fn exit(&mut self) {
        self.push_effect(crate::TerminalEffect::Exit);
    }
    ...
    pub fn toggle_vi_mode(&mut self) {
        ...
        self.push_effect(crate::TerminalEffect::CursorBlinkingChange);
    }
    ...
    pub fn visible_rows(&self) -> Vec<Row<Square>> {
        // Visible rows are the full screen, NOT the scroll region.
        // DECSTBM (`\x1b[t;br`) shrinks `scroll_region` so the
        // terminal scrolls inside a sub-band — but rows outside that
        // band are still drawn (e.g. tail/htop status bars, agent
        // input boxes pinned to the bottom). Restricting visibility
        // to `scroll_region` made those rows render as black voids.
        let screen_lines = self.grid.screen_lines() as i32;
        let mut start = 0i32;
        let mut end = screen_lines;
        ...
        let mut visible_rows = Vec::with_capacity(screen_lines as usize);
    ...
    #[inline]
    pub fn absolute_row_for_line(&self, line: Line) -> usize {
        (self.history_size() as i64 + line.0 as i64).max(0) as usize
    }

    #[inline]
    pub fn visible_row_absolute_indices(&self) -> Vec<usize> {
        ...
        (start..end)
            .map(|row| self.absolute_row_for_line(Line(row)))
            .collect()
    }

    #[inline]
    fn editor_scrollback_array_index(&self, logical_index: isize) -> Option<usize> {
        let len = self.editor_scrollback_lines.len();
        (len > 0).then(|| {
            (self.editor_scrollback_current_index + logical_index)
                .rem_euclid(len as isize) as usize
        })
    }

    #[inline]
    fn set_editor_scrollback_logical(&mut self, logical_index: isize, row: Row<Square>) {
        if let Some(ix) = self.editor_scrollback_array_index(logical_index) {
            self.editor_scrollback_lines[ix] = Some(row);
        }
    }

    /// Flush the current editor viewport into the 2x scrollback ring.
    ///
    /// This intentionally mirrors Neovide's `RenderedWindow::flush`:
    /// rotate by the latest `win_viewport.scroll_delta`, copy the
    /// current inner view into the first half of the ring, and keep the
    /// second half available for signed negative/overflow sampling while
    /// the spring animates. Returns `true` when the ring was already
    /// valid before this flush, which means the caller can animate the
    /// associated scroll delta without showing stale rows.
    pub fn flush_editor_scrollback(&mut self, scroll_delta: i32) -> bool {
        let visible_rows = self.visible_rows();
        let visible_len = visible_rows.len();
        let columns = self.grid.columns();
        let expected_len = visible_len.saturating_mul(2);

        if visible_len == 0 || columns == 0 || expected_len == 0 {
            self.clear_editor_scrollback();
            return false;
        }

        let was_valid = self.editor_scrollback_lines.len() == expected_len
            && self.editor_scrollback_visible_rows == visible_len
            && self.editor_scrollback_columns == columns;

        if !was_valid {
            self.editor_scrollback_lines.clear();
            self.editor_scrollback_lines.reserve(expected_len);
            for _ in 0..2 {
                self.editor_scrollback_lines
                    .extend(visible_rows.iter().cloned().map(Some));
            }
            self.editor_scrollback_current_index = 0;
            self.editor_scrollback_visible_rows = visible_len;
            self.editor_scrollback_columns = columns;
            return false;
        }

        // Keep the logical ring origin unwrapped. Array access applies
        // modulo in `editor_scrollback_array_index`, but render planning
        // compares consecutive origins. Wrapping here turns a normal held
        // arrow scroll into a fake large jump every `expected_len` lines.
        self.editor_scrollback_current_index = self
            .editor_scrollback_current_index
            .saturating_add(scroll_delta as isize);

        for (idx, row) in visible_rows.into_iter().enumerate() {
            self.set_editor_scrollback_logical(idx as isize, row);
        }

        if scroll_delta.unsigned_abs() as usize > visible_len {
            // Neovide's far-scroll path animates through one blank edge
            // line by default. Keep the ring valid but blank the single
            // edge row the spring will sample.
            const FAR_SCROLL_LINES: usize = 1;
            let blank = Row::<Square>::new(columns);
            if scroll_delta > 0 {
                for distance in 1..=FAR_SCROLL_LINES.min(visible_len) {
                    self.set_editor_scrollback_logical(
                        -(distance as isize),
                        blank.clone(),
                    );
                }
            } else {
                for distance in 0..FAR_SCROLL_LINES.min(visible_len) {
                    self.set_editor_scrollback_logical(
                        (visible_len + distance) as isize,
                        blank.clone(),
                    );
                }
            }
    ...
    pub fn editor_scrollback_snapshot(
        &self,
    ) -> Option<(Vec<Option<Row<Square>>>, isize)> {
        let visible_len = self.grid.screen_lines();
        let expected_len = visible_len.saturating_mul(2);
        (visible_len > 0
            && self.editor_scrollback_visible_rows == visible_len
            && self.editor_scrollback_columns == self.grid.columns()
            && self.editor_scrollback_lines.len() == expected_len)
            .then(|| {
                (
                    self.editor_scrollback_lines.clone(),
                    self.editor_scrollback_current_index,
                )
            })
    }

    pub fn editor_scrollback_origin(&self) -> Option<isize> {
        let visible_len = self.grid.screen_lines();
        let expected_len = visible_len.saturating_mul(2);
        (visible_len > 0
            && self.editor_scrollback_visible_rows == visible_len
            && self.editor_scrollback_columns == self.grid.columns()
            && self.editor_scrollback_lines.len() == expected_len)
            .then_some(self.editor_scrollback_current_index)
    }

    pub fn clear_editor_scrollback(&mut self) {
        self.editor_scroll_snapshot_above.clear();
        self.editor_scroll_snapshot_below.clear();
        self.editor_scrollback_lines.clear();
        self.editor_scrollback_current_index = 0;
        self.editor_scrollback_visible_rows = 0;
        self.editor_scrollback_columns = 0;
    }

    pub fn clear_editor_scrollback_edges(
        &mut self,
        clear_above: bool,
        clear_below: bool,
    ) {
        ...
        let visible_len = self.editor_scrollback_visible_rows;
        let expected_len = visible_len.saturating_mul(2);
        if visible_len == 0
            || self.editor_scrollback_lines.len() != expected_len
            || self.editor_scrollback_columns == 0
        ...
        let blank = Row::<Square>::new(self.editor_scrollback_columns);
        if clear_above {
            if let Some(ix) = self.editor_scrollback_array_index(-1) {
                let row = &mut self.editor_scrollback_lines[ix];
                *row = Some(blank.clone());
            }
        }
        if clear_below {
            if let Some(ix) = self.editor_scrollback_array_index(visible_len as isize) {
                let row = &mut self.editor_scrollback_lines[ix];
                *row = Some(blank);
            }
        }
    ...
    pub fn visible_rows_with_damage(
        ...
        // Use the full screen, not `scroll_region` — see the note on
        // `visible_rows()` for why DECSTBM-shrunk regions still need
        // their out-of-region rows drawn.
        let screen_lines = self.grid.screen_lines() as i32;
        let mut start = 0i32;
        let mut end = screen_lines;
        let mut visible_rows = Vec::with_capacity(screen_lines as usize);
    ...
    /// Capture a copyable snapshot of the visible terminal state.
    ///
    /// Walks the active grid's viewport rows resolving each cell's
    /// interned style (fg/bg/flags/underline-colour), the cursor,
    /// modes, title, and damage. Hyperlinks are not yet interned into
    /// the snapshot — that's a future fidelity pass.
    pub fn snapshot(&self) -> crate::snapshot::TerminalSnapshot {
        use crate::colors::{AnsiColor, ColorRgb, NamedColor};
        use crate::crosswords::square::CellFlags as SquareFlags;
        use crate::crosswords::style::StyleFlags;
        use crate::snapshot::{
            CellFlags, CellSnapshot, ColorIndex, CursorShape as SnapShape,
            CursorSnapshot, DamageSnapshot, ModesSnapshot, MouseReporting, RgbTriple,
            TerminalSnapshot, ThemeSnapshot,
        };

        let to_color = |c: AnsiColor| -> ColorIndex {
            match c {
                AnsiColor::Named(NamedColor::Foreground)
                | AnsiColor::Named(NamedColor::Background) => ColorIndex::Default,
                AnsiColor::Named(n) => ColorIndex::Named(n as u8),
                AnsiColor::Indexed(i) => ColorIndex::Indexed(i),
        ...
        let cols = self.grid.columns() as u16;
        let rows = self.grid.screen_lines() as u16;
        let display_offset = self.grid.display_offset();

        let mut viewport: Vec<Vec<CellSnapshot>> = Vec::with_capacity(rows as usize);
        for line_idx in 0..rows as i32 {
            let line = crate::crosswords::pos::Line(line_idx);
            let mut row_cells: Vec<CellSnapshot> = Vec::with_capacity(cols as usize);
            for col_idx in 0..cols as usize {
                let col = crate::crosswords::pos::Column(col_idx);
                let pos = crate::crosswords::pos::Pos::new(line, col);
                ...
                // Per-cell (not per-style) flags: wide-char and wrapline.
                let sq_flags = square.cell_flags();
                if sq_flags.contains(SquareFlags::WRAPLINE) {
                ...
                row_cells.push(CellSnapshot {
                    c: square.c(),
                    fg: to_color(style.fg),
                    bg: to_color(style.bg),
                    flags,
                    underline_color: style.underline_color.map(to_color),
                    hyperlink_id: None,
                });
            }
            viewport.push(row_cells);
        }

        let cursor_pos = self.grid.cursor.pos;
        let cursor_shape = match self.cursor_shape {
            crate::ansi::CursorShape::Block => SnapShape::Block,
            crate::ansi::CursorShape::Beam => SnapShape::Beam,
            crate::ansi::CursorShape::Underline => SnapShape::Underline,
            crate::ansi::CursorShape::Hidden => SnapShape::Hidden,
        };

        // Active palette + well-known slots, emitted on every frame
        // so downstream renderers don't need their own theme copy.
        // `TermColors` stores `Option<[f32; 4]>` per slot (None means
        // "use the renderer's static default"); we convert anything
        // present to an `RgbTriple`, fall back to `ThemeSnapshot`
        // defaults otherwise.
        let theme_default = ThemeSnapshot::default();
        let arr_to_triple = |arr: crate::colors::ColorArray| -> RgbTriple {
        ...
        let palette: Vec<RgbTriple> = (0..256)
            .map(|i| match self.colors[i] {
                Some(arr) => arr_to_triple(arr),
                None => RgbTriple::default(),
            })
            .collect();
        let theme = ThemeSnapshot {
            palette,
            default_fg: self.colors[NamedColor::Foreground as usize]
                .map(arr_to_triple)
                .unwrap_or(theme_default.default_fg),
            default_bg: self.colors[NamedColor::Background as usize]
                .map(arr_to_triple)
                .unwrap_or(theme_default.default_bg),
            cursor: self.colors[NamedColor::Cursor as usize]
                .map(arr_to_triple)
                .unwrap_or(theme_default.cursor),
            selection_bg: theme_default.selection_bg,
            selection_fg: theme_default.selection_fg,
        };
        ...
        TerminalSnapshot {
            ...
            cursor: CursorSnapshot {
                col: cursor_pos.col.0 as u16,
                row: cursor_pos.row.0.max(0) as u16,
                shape: cursor_shape,
                visible: self.mode.contains(Mode::SHOW_CURSOR),
            },
            modes: ModesSnapshot {
                alt_screen: false,
                origin: self.mode.contains(Mode::ORIGIN),
                auto_wrap: self.mode.contains(Mode::LINE_WRAP),
                bracketed_paste: self.mode.contains(Mode::BRACKETED_PASTE),
                focus_events: self.mode.contains(Mode::FOCUS_IN_OUT),
...
impl Handler for Crosswords {
    ...
    fn report_mode(&mut self, mode: AnsiMode) {
        ...
        self.push_effect(crate::TerminalEffect::PtyWrite(
            format!("\x1b[{};{}$y", mode.raw(), state as u8,).into_bytes(),
        ));
    }
    ...
    fn set_progress_report(&mut self, report: crate::ProgressReport) {
        self.push_effect(crate::TerminalEffect::ProgressReport(report));
    }
    ...
    fn semantic_prompt(&mut self, action: SemanticPromptAction) {
        match action {
            SemanticPromptAction::PromptStart => {
                self.shell_prompt.running_command = false;
                self.shell_prompt.awaiting_command = false;
            }
            SemanticPromptAction::PromptEnd => {
                self.shell_prompt.awaiting_command = true;
                self.shell_prompt.running_command = false;
            }
            SemanticPromptAction::CommandStart => {
                self.shell_prompt.awaiting_command = false;
                self.shell_prompt.running_command = true;
            }
            SemanticPromptAction::CommandFinished(exit_code) => {
                self.shell_prompt.awaiting_command = false;
                self.shell_prompt.running_command = false;
    ...
    fn set_cursor_style(&mut self, style: Option<CursorShape>, blinking: bool) {
        if let Some(cursor_shape) = style {
            ...
            // DECSCUSR 0: reset to DEFAULTS — including the config's
            // blink setting. The sequence's blink bit is meaningless
            // here (0 % 2 derives "false", not a user intent), and
            // honoring it let any TUI's exit-reset permanently kill
            // config-enabled blinking.
            self.cursor_shape = self.default_cursor_shape;
            self.blinking_cursor = self.default_blinking_cursor;
        }
    ...
    fn clipboard_store(&mut self, clipboard: u8, base64: &[u8]) {
        ...
        if let Ok(bytes) = general_purpose::STANDARD.decode(base64) {
            if let Ok(text) = simd_utf8::from_utf8_to_string(&bytes) {
                self.push_effect(crate::TerminalEffect::ClipboardStore {
                    ty: clipboard_type.into(),
                    text,
                });
            }
        }
    ...
    fn identify_terminal(&mut self, intermediate: Option<char>) {
        match intermediate {
            None => {
                ...
                self.push_effect(crate::TerminalEffect::PtyWrite(text.into_bytes()));
            }
            Some('>') => {
    ...
    fn set_color(&mut self, index: usize, color: ColorRgb) {
        ...
        self.push_effect(crate::TerminalEffect::ColorChange {
            index,
    ...
    fn reset_color(&mut self, index: usize) {
        ...
        self.push_effect(crate::TerminalEffect::ColorChange { index, color: None });
    }

    #[inline]
    fn bell(&mut self) {
        self.push_effect(crate::TerminalEffect::Bell);
    }

    #[inline]
    fn desktop_notification(&mut self, title: String, body: String) {
        self.push_effect(crate::TerminalEffect::DesktopNotification { title, body });
    }
    ...
    fn clipboard_load(&mut self, clipboard: u8, terminator: &str) {
        ...
        self.push_effect(crate::TerminalEffect::ClipboardLoad {
            ty: clipboard_type.into(),
            clipboard_byte: clipboard,
            terminator: terminator.to_owned(),
        });
    }
    ...
    fn text_area_size_pixels(&mut self) {
        ...
        self.push_effect(crate::TerminalEffect::TextAreaSizeRequest {
            kind: crate::TextAreaSizeRequestKind::Pixels,
    ...
    fn graphics_attribute(&mut self, pi: u16, pa: u16) {
        ...
        let (ps, pv) = match pi {
            ...
            2 => {
                match pa {
                    1 => {
                        self.push_effect(crate::TerminalEffect::TextAreaSizeRequest {
                            kind: crate::TextAreaSizeRequestKind::GraphicsAttribute {
                                pi,
                            },
                            terminator: String::new(),
                        });
        ...
        self.push_effect(crate::TerminalEffect::PtyWrite(
            generate_response(pi, ps, pv).into_bytes(),
        ));
    }
    ...
    fn kitty_graphics_response(&mut self, response: String) {
        ...
        self.push_effect(crate::TerminalEffect::PtyWrite(response.into_bytes()));
    }
...
impl Crosswords {
    ...
    fn place_kitty_overlay(
        ...
        if placement.columns > 0 || placement.rows > 0 {
            ...
            graphic_data.resize = Some(crate::graphics::ResizeCommand {
                width: if placement.columns > 0 {
                    crate::graphics::ResizeParameter::Cells(placement.columns)
                } else {
                    crate::graphics::ResizeParameter::Auto
                },
                height: if placement.rows > 0 {
                    crate::graphics::ResizeParameter::Cells(placement.rows)
                } else {
                    crate::graphics::ResizeParameter::Auto
                },
...
mod tests {
    ...
    fn set_first_col(cw: &mut Crosswords, chars: &[char]) {
        for (row, ch) in chars.iter().copied().enumerate() {
            cw.grid[Line(row as i32)][Column(0)].set_c(ch);
        }
    }

    fn scrollback_char_at(
        snapshot: &(Vec<Option<Row<Square>>>, isize),
        logical_index: isize,
    ) -> char {
        let (rows, origin) = snapshot;
        let ix = (*origin + logical_index).rem_euclid(rows.len() as isize) as usize;
        rows[ix].as_ref().unwrap()[Column(0)].c()
    }

    #[test]
    fn editor_scrollback_uses_logical_ring_indices_for_scroll_down() {
        let size = CrosswordsSize::new(1, 4);
        let mut cw =
            Crosswords::new(size, CursorShape::Block, crate::TerminalId::new(0), 10_000);

        set_first_col(&mut cw, &['0', '1', '2', '3']);
        assert!(!cw.flush_editor_scrollback(0));

        set_first_col(&mut cw, &['1', '2', '3', '4']);
        assert!(cw.flush_editor_scrollback(1));

        let snapshot = cw.editor_scrollback_snapshot().unwrap();
        assert_eq!(scrollback_char_at(&snapshot, -1), '0');
        assert_eq!(scrollback_char_at(&snapshot, 0), '1');
        assert_eq!(scrollback_char_at(&snapshot, 3), '4');
    }

    #[test]
    fn editor_scrollback_origin_does_not_wrap_logically() {
        let size = CrosswordsSize::new(1, 4);
        let mut cw =
            Crosswords::new(size, CursorShape::Block, crate::TerminalId::new(0), 10_000);
        ...
        for step in 1..=9 {
            set_first_col(
                &mut cw,
                &[
                    char::from(b'0' + step as u8),
                    char::from(b'1' + step as u8),
                    char::from(b'2' + step as u8),
                    char::from(b'3' + step as u8),
                ],
            );
            assert!(cw.flush_editor_scrollback(1));
        }
        ...
        assert_eq!(scrollback_char_at(&snapshot, -1), '8');
        assert_eq!(scrollback_char_at(&snapshot, 0), '9');
    }

    #[test]
    fn editor_scrollback_uses_logical_ring_indices_for_scroll_up() {
        let size = CrosswordsSize::new(1, 4);
        let mut cw =
            Crosswords::new(size, CursorShape::Block, crate::TerminalId::new(0), 10_000);
        ...
        let snapshot = cw.editor_scrollback_snapshot().unwrap();
        assert_eq!(scrollback_char_at(&snapshot, 0), '0');
        assert_eq!(scrollback_char_at(&snapshot, 3), '3');
        assert_eq!(scrollback_char_at(&snapshot, 4), '4');
    }
    ...
    fn osc8_hyperlink_basic() {
        use crate::handler::{Processor, StdSyncHandler};
    ...
    fn make_term_for_selection(rows: usize, cols: usize) -> Crosswords {
        let size = CrosswordsSize::new(cols, rows);
        Crosswords::new(size, CursorShape::Block, crate::TerminalId::new(0), 10_000)
    }

    fn select_simple(term: &mut Crosswords, start: (i32, usize), end: (i32, usize)) {
        term.selection = Some(Selection::new(
            SelectionType::Simple,
            Pos {
                row: Line(start.0),
    ...
    fn test_xtversion_report() {
        ...
        // Verify that a PtyWrite effect was pushed
        let effects: Vec<_> = term.drain_effects().collect();
        assert_eq!(effects.len(), 1, "Should have pushed one effect");

        // Verify the effect is PtyWrite with the correct format
        match &effects[0] {
            crate::TerminalEffect::PtyWrite(bytes) => {
                let text = std::str::from_utf8(bytes).expect("utf-8 PtyWrite");
    ...
    fn sixel_stores_graphic_in_extras_table() {
        ...
        let graphic = GraphicData {
            id: crate::graphics::GraphicId::new(0), // will be reassigned
            width: 20,
            height: 20,
            pixels: vec![0u8; 20 * 20 * 4],
            color_type: crate::graphics::ColorType::Rgba,
            is_opaque: true,
            display_width: None,
            display_height: None,
            resize: None,
    ...
    fn cell_graphic_accessor() {
        ...
        let graphic = GraphicData {
            id: crate::graphics::GraphicId::new(0),
            width: 10,
            height: 10,
    ...
    fn new_term(cols: usize, rows: usize) -> Crosswords {
        let size = CrosswordsSize::new(cols, rows);
        Crosswords::new(size, CursorShape::Block, crate::TerminalId::new(0), 10_000)
    }
    ...
    fn icat_unicode_placeholder_wire_sequence_lands_in_grid() {
        ...
        for (row, &row_diac) in DIACRITICS.iter().enumerate().take(rows as usize) {
            for (col, &col_diac) in DIACRITICS.iter().enumerate().take(cols as usize) {
                ...
                match style.fg {
                    crate::colors::AnsiColor::Spec(rgb) => {
