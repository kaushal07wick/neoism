...
impl Grid<Square> {
    ...
    #[inline]
    pub fn blank_with_bg(&mut self, bg: crate::colors::AnsiColor) -> Square {
        use crate::colors::{AnsiColor, NamedColor};
...
/// Test-friendly `Dimensions` for `(rows, columns)` tuples.
///
/// Kept un-gated (was `#[cfg(test)]` before phase 3b) so the backend
/// crate's *own* test suite — which lives in a different compilation
/// unit — can also pass a tuple to `Crosswords::new`. Production code
