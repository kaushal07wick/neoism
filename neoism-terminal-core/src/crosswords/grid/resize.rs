...
impl Grid<Square> {
    ...
    pub fn resize(&mut self, reflow: bool, lines: usize, columns: usize) {
        ...
        match self.lines.cmp(&lines) {
            Ordering::Less => self.grow_lines(reflow, lines),
    ...
    fn grow_lines(&mut self, reflow: bool, target: usize) {
        ...
        // Primary screen (reflow=true): move existing lines up for every row
        // that couldn't be pulled from scrollback history.
        // Alt-screen (reflow=false): skip the scroll — new blank rows already
        // landed at the bottom; the TUI will fully redraw on SIGWINCH.
        if reflow && from_history != lines_added {
