.PHONY: docs dev-isolated dev-isolated-build-debug dev-isolated-release dev-isolated-build
...
run:
	cargo run -p neoism --release

# OXS: optionally you can run "/bin/launchctl setenv MTL_HUD_ENABLED 1"
dev:
	MTL_HUD_ENABLED=1 cargo run -p neoism --features wgpu

dev-debug:
	MTL_HUD_ENABLED=1 NEOISM_LOG_LEVEL=debug make dev

dev-debug-wayland:
	NEOISM_LOG_LEVEL=debug cargo run -p neoism --no-default-features --features=wayland

dev-debug-x11:
	NEOISM_LOG_LEVEL=debug cargo run -p neoism --no-default-features --features=x11

dev-isolated:
	./scripts/dev-neoism.sh dev

dev-isolated-build-debug:
	./scripts/dev-neoism.sh build-dev

dev-isolated-release:
	./scripts/dev-neoism.sh release

dev-isolated-build:
	./scripts/dev-neoism.sh build

run-wasm:
	cargo build -p neoism --target wasm32-unknown-unknown --lib
	cargo run -p neoism-wasm

dev-watch:
	#cargo install cargo-watch
	cargo watch -- cargo run -p neoism
...
app-universal: $(APP_NAME)-universal ## Create a universal Neoism.app
$(APP_NAME)-%: $(TARGET)-%
...
release-macos-signed-app:
	...
	@ditto -c -k --keepParent ./release/$(APP_NAME) ./release/neoism-v$(version).zip
	@xcrun notarytool submit ./release/neoism-v$(version).zip --keychain-profile "TODO_NOTARY_PROFILE" --wait
	rm -rf ./release/$(APP_NAME)
	@unzip ./release/neoism-v$(version).zip -d ./release
	@echo "Please verify if 'Neoism.app/Contents/Resources/72/neoism' exists before create-dmg"
...
version-not-found:
	@echo "Neoism version was not specified"
...
# 	Using https://www.npmjs.com/package/create-dmg
	cd ./release && create-dmg $(APP_NAME) --dmg-title="Neoism ${version}" --overwrite

bump-brew:
	brew bump-cask-pr neoism --version ${version}
...
# To install: sudo release/debian/neoism_<version>_<architecture>_<feature>.deb
# e.g: sudo release/debian/neoism_0.0.13_arm64_wayland.deb
release-debian-x11:
	cargo deb -p neoism -- --no-default-features --features=x11
...
release-debian-wayland:
	cargo deb -p neoism -- --no-default-features --features=wayland
...
install-debian-x11:
	cargo install cargo-deb
	cargo deb -p neoism --install -- --release --no-default-features --features=x11
install-debian-wayland:
	cargo install cargo-deb
	cargo deb -p neoism --install -- --release --no-default-features --features=wayland
...
# Guard: fail if anyone adds new UI code to the desktop fork that should
# live in the shared `neoism-ui` crate. See the script header for the
# rule and the ALLOW_DESKTOP_UI=1 bypass.
check-ui-fork:
	@./scripts/check_no_ui_in_desktop_fork.sh
