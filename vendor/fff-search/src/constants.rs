...
/// Minimum file size for the persistent content cache (named for its mmap
/// origins; the cache now uses heap reads on every platform). Sized to one
/// page so sub-page files fall back to chunked reads.
