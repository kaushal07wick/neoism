...
pub struct FileItem {
    ...
    flags: AtomicU8,
    /// Lazy heap-backed content cache. File-backed mmaps can SIGBUS if a file
    /// is truncated while search is reading it.
...
impl FileItem {
    ...
    ///
    /// Returns a reference to cached heap-backed file contents.
...
/// Retained for grep worker scratch-state API compatibility. Neoism's patched
/// FFF build avoids file-backed mmaps so concurrent rewrites cannot SIGBUS.
pub type MmapSlot = ();
