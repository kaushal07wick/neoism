...
use neoism_terminal_core::crosswords::pos::Side;
...
pub struct Mouse {
    ...
    pub raw_y: f64,
    /// Raw (unclamped) cursor X in physical pixels. Winit captures the
    /// pointer in the source window for the whole duration of a tab
    /// drag, so when the cursor leaves the window's inner bounds the
    /// clamped `x` saturates at the edge. The cross-window drop
...
pub fn calculate_mouse_position(
    ...
    // update_scaled_margin), so use them directly. Keep the math in
    // float space: active terminal panes can shift their grid origin
    // above the pane while the composer is visible, which makes the
    // effective top margin negative.
    let x_in_grid = mouse.x as f32 - margin_x_left;
    let col = if x_in_grid < cell_width {
        0
    } else {
        (x_in_grid / cell_width).floor() as usize
    };
    let col = Column(col.min(columns_rows.0.saturating_sub(1)));

    let y_in_grid = mouse.y as f32 - margin_y_top;
    let row = if y_in_grid <= 0.0 {
        0
    } else {
        (y_in_grid / cell_height).floor() as usize
    };
    let calc_row = row.min(columns_rows.1.saturating_sub(1));
...
pub fn calculate_side_by_pos(
    ...
    neoism_ui::mouse_policy::cell_side_by_pos(x, margin_x, cell_width, grid_width)
}
...
pub mod test {
    ...
    /// Active terminal panes shift the rendered grid upward while the
    /// floating composer is visible. That makes the effective top margin
    /// negative; clamping it to zero maps mouse selection/hover several
    /// rows above the text under the cursor.
    #[test]
    fn test_negative_top_margin_from_composer_shift() {
        let mouse = Mouse {
            x: 20,
            y: 90,
            ..Default::default()
        };
        let pos = calculate_mouse_position(&mouse, 0, (80, 20), 0.0, -90.0, (10.0, 18.0));

        assert_eq!(pos.row, Line(10));
    }
