...
impl Screen<'_> {
    pub(crate) fn snapshot_panels(&mut self, ctx: &mut FrameCtx) {
        ...
        // `ghostty/src/renderer/generic.zig:2431-2440`.
                ...
                .iter_mut()
            {
                ...
                let editor_rows_override = ctx.editor.as_ref().and_then(|_| {
                ...
                if let Some(editor_rows) = editor_rows_override {
                    let visible_cap = (editor_rows as usize).max(1);
                    ...
                    && !visible_rows.is_empty()
                {
                    ...
                    let overflow =
                        content_rows.len().saturating_sub(terminal_content_rows);
                    if overflow > 0 {
                        let trailing_empty = content_rows
                            ...
                            .count();
                        if display_offset >= history_size as i32
                            || trailing_empty >= overflow
                        {
                    ...
                    let content_anchor_abs =
                        content_sources.first().copied().unwrap_or(0);
                    ...
                    let before =
                        terminal_content_rows.saturating_mul(3).saturating_add(16);
                    let after =
                        terminal_content_rows.saturating_mul(3).saturating_add(16);
                ...
                let editor_has_render_offset = editor_render_offset.source_line_offset
                    != 0
                ...
                panels.push(PanelFrame {
                    ...
                    cursor_visible: terminal_cursor_visible(
                        TerminalCursorVisibilityInput {
                            block_footer_active,
                            is_active,
            ...
            self.renderer.terminal_block_prompt_animating =
                terminal_block_prompt_animating;
    ...
    pub(crate) fn mask_editor_seams(&mut self, ctx: &FrameCtx) {
            ...
            // Mask chrome/status seams around editor panes with the
            // active theme background. Without this, the GPU grid's buffer rows
            // (rendered above/below visible) peek through the safety
            // pads — visible to the user as "half a line above the
            // first line / below the last line" of the editor pane.
            ...
            // grid pass per `render_wgpu`, so it covers the bleed.
            {
                ...
                let status_h_logical = self.renderer.status_line_height();
                for p in &ctx.panels {
                    if !p.is_editor {
                        continue;
                    }
                    let panel_left_phys = scaled_margin.left + p.layout_rect[0];
                    let panel_w_phys = p.cols as f32 * p.cell_w;
                    let pane_top_phys = scaled_margin.top + p.layout_rect[1];
                    let pane_h_phys = p.rows as f32 * p.cell_h;
                    let pane_bottom_phys = pane_top_phys + pane_h_phys;

                    // Safety masks must stay inside the editor's chrome
                    // zone/panel. A split below the editor is a real
                    // terminal pane; painting from the editor bottom to
                    // the global status line blanks that terminal.
                    let mask_color = self.renderer.theme.f32(self.renderer.theme.bg);
                    let win_h = self.sugarloaf.window_size().height as f32 / s;
                    for mask in editor_chrome_mask_rects(EditorChromeMaskInput {
                        ...
                        pane_bottom: pane_bottom_phys / s,
                        panel_bottom: (scaled_margin.top
                            + p.layout_rect[1]
                            + p.layout_rect[3])
                            / s,
                        chrome_height: chrome_height_logical,
                        status_line_height: status_h_logical,
                        window_height: win_h,
                    }) {
                        self.sugarloaf.rect(
                            None,
