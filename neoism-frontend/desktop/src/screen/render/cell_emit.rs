        ...
        let scale_factor = ctx.scale_factor;
            ...
            for (route_id, grid) in self.grids.iter_mut() {
                ...
                let previous_scroll_state =
                    self.editor_scroll_grid_states.get(&route_key);
                ...
                let editor_source_changed =
                    p.is_editor && editor_frame_plan.source_changed;
                ...
                let mut fg_scratch: Vec<neoism_backend::sugarloaf::grid::CellText> =
                    Vec::with_capacity(cols);
                let mut hint_scratch: Vec<crate::terminal::grid_emit::RowHint> =
                    Vec::new();
                ...
                let mut shifted_rows = editor_scroll_shifted_row_count(
                    editor_source_plan,
                    p.visible_rows.len(),
                );

                if let EditorScrollSourcePlan::Shift { delta, exposed } =
                    editor_source_plan
                {
                ...
                if p.is_editor {
                    ...
                    let damaged_source = |source_y: i32| {
                        if let neoism_terminal_core::damage::TerminalDamage::Partial(
                            lines,
                        ) = &p.damage
                        {
                ...
                {
                    let status_h_px = self.renderer.status_line_height()
                    ...
                    if self.editor_geometry_log_last.get(&p.route_id).copied()
                        != Some(state)
                    {
                ...
                if p.is_editor {
                    let should_log_scroll = editor_scroll_was_animating
                        ...
                        || editor_pixel_changed;
                    let state =
                        self.editor_scroll_grid_states.entry(route_key).or_default();
                    let retained_origin =
                    ...
                    if should_log_scroll {
                        ...
                        //   RedrawRequested.
                        let render_us =
                            render_started.elapsed().as_micros().min(u64::MAX as u128)
                                as u64;
                        ...
                        let source_step_lines = previous_source_base
                            .map(|previous| {
                                (current_source_base - previous).unsigned_abs()
                            })
                            .unwrap_or(0)
                            .min(u32::MAX as u64)
                            as u32;
                        let scroll_log_enabled =
                            std::env::var_os(SCROLL_LOG_ENV).is_some();
                        ...
                        state.log_render_us =
                            state.log_render_us.saturating_add(render_us);
