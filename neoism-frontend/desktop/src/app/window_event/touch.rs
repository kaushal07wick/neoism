...
use neoism_window::event::{ElementState, MouseButton, Touch, TouchPhase};
...
fn on_touch_start(
    route: &mut Route,
    touch: Touch,
    _clipboard: &mut neoism_backend::clipboard::Clipboard,
) {
        ...
        TouchPurpose::Tap(start) => {
            ...
            if delta_x.abs() > MAX_TAP_DISTANCE {
                ...
                *touch_purpose = TouchPurpose::Select(*start);
                ...
                route.window.screen.on_left_click(
                    route
                        .window
                        .screen
                        .terminal_body_mouse_position(0)
                        .or(Some(route.window.screen.mouse_position(0))),
                    clipboard,
                );
