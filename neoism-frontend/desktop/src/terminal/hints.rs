use neoism_backend::config::hints::Hint;
...
impl HintState {
    ...
    pub fn update_matches(
        &mut self,
        term: &neoism_terminal_core::crosswords::Crosswords,
    ) {
        self.matches.clear();
    ...
    pub fn keyboard_input(
        ...
        // Shared decision: pure label-matching state machine. The
        // mutation (push key / pop key / stop / fire) still lives
        // here because `HintState` is desktop-owned.
        let persist = self
            .active_hint
            .as_ref()
            .map(|h| h.persist)
        ...
        match neoism_ui::selection_input::hint_keystroke_decision(
            c,
            &visible_labels,
            persist,
        ) {
            neoism_ui::selection_input::HintKeystrokeDecision::PopKey => {
                self.keys.pop();
                // Only update matches after backspace to regenerate visible labels
                self.update_matches(term);
                None
            }
            neoism_ui::selection_input::HintKeystrokeDecision::StopHintMode => {
                self.stop();
                None
            }
            neoism_ui::selection_input::HintKeystrokeDecision::FireMatch {
                match_index,
                persist,
            } => {
            ...
            neoism_ui::selection_input::HintKeystrokeDecision::PushKey => {
                self.keys.push(c);
                None
            }
            neoism_ui::selection_input::HintKeystrokeDecision::Ignore => None,
        }
    }
...
mod tests {
    use super::*;
    use neoism_backend::config::hints::{HintAction, HintInternalAction};
        ...
        state.matches = vec![
            HintMatch {
                ...
                start: neoism_terminal_core::crosswords::pos::Pos::new(
                    neoism_terminal_core::crosswords::pos::Line(0),
                    neoism_terminal_core::crosswords::pos::Column(0),
                ),
                end: neoism_terminal_core::crosswords::pos::Pos::new(
                    neoism_terminal_core::crosswords::pos::Line(0),
                    neoism_terminal_core::crosswords::pos::Column(5),
                ),
                hint: Rc::new(Hint {
                    regex: Some("test".to_string()),
                    hyperlinks: false,
            ...
            HintMatch {
                ...
                start: neoism_terminal_core::crosswords::pos::Pos::new(
                    neoism_terminal_core::crosswords::pos::Line(0),
                    neoism_terminal_core::crosswords::pos::Column(10),
                ),
                end: neoism_terminal_core::crosswords::pos::Pos::new(
                    neoism_terminal_core::crosswords::pos::Line(0),
                    neoism_terminal_core::crosswords::pos::Column(15),
                ),
                hint: Rc::new(Hint {
                    regex: Some("test".to_string()),
                    hyperlinks: false,
    ...
    fn test_resolve_path_expands_env_var() {
        ...
        // unsafe in Rust 2024; neoism-backend uses an earlier edition so it's
