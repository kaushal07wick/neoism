...
impl ChromeBridge {
        ...
        pub fn agent_handle_key(
            ...
            use neoism_ui::panels::agent_pane::bridge_policy::{
                ...
                AgentBridgeModifiers, AgentKeyContext, AgentKeyIntent,
            ...
            let event =
                AgentBridgeKeyEvent {
                    ...
                    logical_key,
                    key_without_modifiers: agent_bridge_key_from_web(
                        if text.is_empty() { key } else { text },
                    ),
            ...
            for intent in decision.intents {
                match intent {
                    ...
                    AgentKeyIntent::MoveInputUpOrHistory => {
                    ...
                    AgentKeyIntent::RespondPendingPermission(reply) => {
                        let choice = match reply {
                            AgentPermissionReply::Once => {
                                NeoismAgentPermissionChoice::Once
                            }
        ...
        pub fn agent_input_rect_json(&mut self) -> JsValue {
            use neoism_ui::panels::agent_pane::view::{
                layout as agent_layout, side_panel,
            };
        ...
        pub fn agent_send_message_with_attachments(
            ...
            let attachments: Vec<Attachment> = serde_json::from_str(attachments_json)
                .map_err(|e| {
                    JsValue::from_str(&format!("agent attachments parse: {e}"))
                })?;
            let trimmed = text.trim();
        ...
        pub fn drain_agent_outbound(&mut self) -> u32 {
            ...
            let commands = match self.chrome.agent_pane_mut() {
                Some(pane) if pane.has_pending_outbound() => {
