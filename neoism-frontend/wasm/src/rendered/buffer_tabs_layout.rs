...
use neoism_ui::panels::buffer_tabs::{
    apply_buffer_tab_policy as shared_apply_buffer_tab_policy,
    BufferTabPolicyInput, BufferTabPolicyOperation,
};
...
impl ChromeBridge {
                ...
                .map(|(ix, t)| {
                    ...
                    BufferTab {
                        ...
                        markdown: t
                            ...
                            .unwrap_or(false),
                        scratch_id: None,
                        terminal_route_id: (t.kind.as_deref() == Some("terminal")
                            && ix != 0)
                            .then_some(ix),
        ...
        pub fn apply_buffer_tab_policy(
            ...
            let operation = match operation {
                ...
                "reorder" => {
                    let packed = index.ok_or_else(|| {
                        JsValue::from_str("reorder requires packed index")
                    })?;
        ...
        pub fn apply_session_layout_policy(
            ...
            fn push_pane_rects(
                ...
                match layout.node(node_id).ok_or_else(|| {
                    JsValue::from_str("layout references a missing node")
                })? {
            ...
            let mut layout =
                if let Some(json) = state_json.filter(|json| !json.is_empty()) {
            ...
            match operation {
                ...
                "split" => {
                    layout
                        ...
                        .map_err(|e| {
                            JsValue::from_str(&format!("layout split: {e:?}"))
                        })?;
                }
                "focus_next" => {
                    layout.focus_adjacent_leaf(false, true).map_err(|e| {
                        JsValue::from_str(&format!("layout focus: {e:?}"))
                    })?;
                }
                "focus_prev" => {
                    layout.focus_adjacent_leaf(true, true).map_err(|e| {
                        JsValue::from_str(&format!("layout focus: {e:?}"))
                    })?;
                }
                "focus_external" => {
                ...
                "ensure_external" => {
                    ...
                    if !already {
                        layout
                            ...
                            .map_err(|e| {
                                JsValue::from_str(&format!(
                                    "layout ensure_external: {e:?}"
                                ))
                            })?;
                    }
                }
                "close_focused" => {
                    layout.close_focused_leaf().map_err(|e| {
                        JsValue::from_str(&format!("layout close: {e:?}"))
                    })?;
                }
                "resize" => {
                    ...
                    layout
                        .resize_split_toward_leaf(
                            layout.focused_leaf(),
                            Some(axis),
                            delta,
                        )
                        .map_err(|e| {
                            JsValue::from_str(&format!("layout resize: {e:?}"))
                        })?;
                }
                other => {
                    return Err(JsValue::from_str(&format!(
