...
use neoism_ui::render_policy::{
    block_status_color_token,
    loader_animation_frame, loader_orbit_position, loader_pastel_color, BlockStatusColorToken,
};
use neoism_ui::terminal_blocks::BlockStatusKind;
    ...
    pub(crate) fn palette_action_name(
        ...
        match action {
            ...
            A::SendCurrentWorkspaceToDockerSandbox => {
    ...
    pub(crate) fn generate_crdt_client_id() -> u64 {
        loop {
            let id = (js_sys::Math::random() * 9_007_199_254_740_992.0) as u64
                & ((1 << 53) - 1);
            if id != 0 {
                return id;
            }
