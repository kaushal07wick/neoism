...
use neoism_ui::{
    EditorScrollViewportBounds, PanelKey,
};
...
impl ChromeBridge {
        ...
        pub fn drain_top_bar_action(&mut self) -> Option<String> {
            self.chrome
                ...
                .map(|action| match action {
                    ...
                    neoism_ui::panels::TopBarAction::OpenThemes => {
                        "open_themes".to_string()
                    }
        ...
        pub(crate) fn editor_add_wheel_delta(&mut self, delta_pixels: f32, cell_height: f32) -> i32 {
        ...
        pub fn editor_pointer_intent(&self, x: f32, y: f32) -> JsValue {
            ...
            match self.chrome.editor_grid_hit_cell(x, y) {
                Some((row, col)) => serde_wasm_bindgen::to_value(&Cell { row, col })
                    .unwrap_or(JsValue::NULL),
                None => JsValue::NULL,
            }
        }
