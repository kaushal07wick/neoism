use neoism_ui::Chrome;
    ...
    pub(crate) fn cursor_shape_for_nvim_mode(mode: &str) -> neoism_terminal_core::ansi::CursorShape {
        let mode = mode.trim().to_ascii_lowercase();
        if mode.starts_with("insert") || mode == "i" || mode == "ic" || mode == "ix" {
            neoism_terminal_core::ansi::CursorShape::Beam
        } else if mode.starts_with("replace")
            || mode == "r"
            || mode == "rx"
            || mode == "rvc"
        {
    ...
    pub(crate) fn base64_encode(bytes: &[u8]) -> String {
        ...
        for chunk in bytes.chunks(3) {
            ...
            if chunk.len() > 1 {
                out.push(
                    B64_ALPHABET[(((b1 & 0b1111) << 2) | (b2 >> 6)) as usize] as char,
                );
            } else {
                out.push('=');
            }
        ...
        for provider in providers {
            ...
            for model in &provider.models {
                let footer = model
                    .context_limit
                    .map(|limit| {
                        format!("{}k ctx", (limit as f32 / 1000.0).round() as u64)
                    })
                    .unwrap_or_default();
