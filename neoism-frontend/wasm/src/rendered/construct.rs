...
use neoism_ui::panels::{
    DiagnosticCounts, FileTree, GitChangeSummary, Mode,
    PrimaryKind, StatusInfo,
};
use neoism_ui::terminal_blocks::TerminalInputBuffer;
use neoism_ui::widgets::island::Island;
use neoism_ui::{
    Chrome, CustomCursor, GitBranch,
};
...
impl ChromeBridge {
        ...
        pub async fn new(
            ...
            let viewport =
                ChromeRect::new(0.0, 0.0, cols as f32 * 8.0, rows as f32 * 16.0);
        ...
        pub fn enter_palette_fonts_mode(
            &mut self,
            fonts_json: &str,
        ) -> Result<(), JsValue> {
        ...
        pub fn enter_palette_shaders_mode(
            ...
            let shaders: Vec<neoism_ui::panels::command_palette::PaletteShaderEntry> =
                serde_json::from_str(shaders_json).map_err(|e| {
                    JsValue::from_str(&format!("palette shaders parse: {e}"))
                })?;
        ...
        pub fn open_workspaces_palette(
            &mut self,
            payload_json: &str,
        ) -> Result<(), JsValue> {
