        ...
        pub async fn new(
            ...
            let font_library = FontLibrary::default();
            {
                ...
                *lib = FontLibraryData::default();
                ...
                lib.insert(
                    FontData::from_static_slice(FONT_GEIST_MONO_BOLD_ITALIC).unwrap(),
                );

                let nerd_symbols_id = lib.inner.len();
                lib.insert(
                    FontData::from_static_slice(FONT_SYMBOLS_NERD_FONT_MONO).unwrap(),
                );
            ...
            let mut sugarloaf = Sugarloaf::from_canvas(
                canvas,
                size,
                scale,
                renderer,
            ...
            sugarloaf.set_background_color(Some(
                initial_ide_theme.sugar(initial_ide_theme.bg),
            ));
            ...
            let cell_rows: Vec<Vec<CellSnapshot>> = rows
                ...
                .map(|row| {
                    row.inner
                        .iter()
                        .map(|square| {
                            square_to_cell_snapshot(&self.terminal.inner, square)
                        })
                        .collect()
                })
                .collect();
