...
use neoism_ui::render_policy::{
    block_header_panel_geometry, block_header_row_metrics,
    block_status_glyph,
    BlockHeaderPanelGeometryInput, GridPanelGeometry,
    ScaledMargin,
};
use neoism_ui::terminal_blocks::{
    compose_block_chrome_window, compose_block_chrome_window_pinned_bottom,
    display_path, row_is_empty, BlockHeaderSpan, BlockStatusKind,
    CommandBlockSnapshot, COMMAND_BLOCK_COMMAND_ROW,
    COMMAND_BLOCK_META_ROW,
};
...
impl ChromeBridge {
        ...
        pub(crate) fn sync_terminal_status_cwd(&mut self, cwd: Option<&Path>) {
            ...
            if self.chrome.status_line.info().cwd_label.as_deref()
        ...
        pub(crate) fn draw_terminal_blocks_or_cells(
            ...
            let (mut raw_rows, mut raw_sources, display_offset, history_size, cursor_abs) = {
                ...
                let cursor_abs =
                    terminal.absolute_row_for_line(terminal.grid.cursor.pos.row);
                (
                    raw_rows,
                    raw_sources,
            ...
            let terminal_content_rows =
                viewport_rows.saturating_sub(composer_rows).max(1);
        ...
        pub(crate) fn draw_terminal_block_headers(
            ...
            let animation_phase = ((self.services_state.0.borrow().now_ms / 1000.0)
                as f32)
                .rem_euclid(10_000.0);
            ...
            for span in spans {
                ...
                for display_row in span.start_display_row..span.end_display_row {
                    ...
                    if chrome_row == COMMAND_BLOCK_META_ROW {
                        ...
                        let glyph_w = if let Some(glyph) =
                            block_status_glyph(block.status)
                        {
