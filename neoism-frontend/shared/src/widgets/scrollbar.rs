...
pub const SCROLLBAR_HIT_WIDTH: f32 = 14.0;
...
/// for modal UI that also draws its shell/text as late overlays.
#[allow(clippy::too_many_arguments)]
pub fn draw_thumb_overlay(
    sugarloaf: &mut Sugarloaf,
    x: f32,
    y: f32,
    height: f32,
...
impl Scrollbar {
    ...
    pub fn needs_redraw(&mut self) -> bool {
        ...
        self.last_scroll_times.retain(|(_, t)| {
            Instant::now().saturating_duration_since(*t).as_millis() < deadline
        });
...
mod tests {
    ...
    fn opacity_one_while_dragging_regardless_of_last_scroll() {
        ...
        let old = Instant::now() - web_time::Duration::from_secs(10);
    ...
    #[test]
    fn terminal_thumb_does_not_exceed_short_track() {
        let geom =
            Scrollbar::compute_thumb([0.0, 0.0, 80.0, 16.0], 1.0, 0, 100, 8, (0.0, 0.0));
        assert!(geom.thumb_height <= geom.track_height);
    }
