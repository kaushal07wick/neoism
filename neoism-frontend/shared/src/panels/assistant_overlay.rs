...
//! Top-right warning / error overlay.
//!
//! Pure visual panel — lifted from `frontends/neoism/src/neoism/
//! assistant_overlay.rs` so the web frontend can paint the same
//! overlay. The desktop fork re-exports this type and supplies a
//! convenience wrapper that converts `neoism_backend::RioError` into
//! [`AssistantOverlayPayload`] before calling [`AssistantOverlay::
//! set_payload`].
//!
//! The overlay does not own any host concerns (no clipboard / network /
//! filesystem) — it only knows how to lay itself out, hit-test the
//! close + docs-link buttons, and paint via `sugarloaf`. The host
//! decides what to do with the actions it returns.
...
/// Whether the overlay is showing an error or a warning. Drives the
/// heading colour + label only.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AssistantOverlayLevel {
    Warning,
    Error,
}

/// Plain payload the overlay paints. The desktop fork builds one of
/// these from `neoism_backend::RioError`; the web frontend can build
/// one from whatever its error vocabulary is.
#[derive(Debug, Clone)]
pub struct AssistantOverlayPayload {
    pub level: AssistantOverlayLevel,
    /// Multi-line report. The overlay caps the visible line count to
    /// [`MAX_VISIBLE_LINES`] and renders one [`LINE_HEIGHT`] per line.
    pub message: String,
}
...
pub struct AssistantOverlay {
    payload: Option<AssistantOverlayPayload>,
...
impl AssistantOverlay {
    ...
    #[inline]
    pub fn set_payload(&mut self, payload: AssistantOverlayPayload) {
    ...
    fn body_line_count(&self) -> usize {
        if let Some(payload) = &self.payload {
            payload.message.lines().count().max(1)
        } else {
            0
        }
    }
    ...
    pub fn render(&mut self, sugarloaf: &mut Sugarloaf, dimensions: (f32, f32, f32)) {
        ...
        let is_error = payload.level == AssistantOverlayLevel::Error;
        ...
        let report_text = payload.message.clone();
