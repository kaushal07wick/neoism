    ...
    fn classify_strip_click_pane_close_routes_through_pane_strip() {
        let outcome =
            classify_strip_click(Some((4, TabHit::Close(2))), None, None, 0.0, 0.0);
        assert_eq!(
            outcome,
    ...
    fn classify_strip_click_absorbs_misses_inside_workspace_strip() {
        let outcome =
            classify_strip_click(None, Some(workspace_geom()), None, 500.0, 48.0);
    ...
    fn classify_strip_click_passes_through_outside_strip_rect() {
        let outcome =
            classify_strip_click(None, Some(workspace_geom()), None, 10.0, 10.0);
    ...
    fn drop_preview_update_emits_when_dest_differs_from_source() {
        let upd =
            drop_preview_update(StripKey::Workspace, Some(StripKey::Pane(3)), 123.0);
        assert_eq!(
            upd,
