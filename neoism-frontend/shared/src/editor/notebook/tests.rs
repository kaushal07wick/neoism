    ...
    fn appends_streaming_output_chunks() {
        ...
        assert!(pane.markdown.lines.iter().any(
            |line| line == "%%neoism_notebook_output _ _ neoism_state=running hello"
        ));
    }
    ...
    fn streaming_output_follows_cell_id_after_reorder() {
        ...
        let job = pane.prepare_cell_execution(0).unwrap();
        let first_range =
            pane.cell_ranges[0].line_start..pane.cell_ranges[0].line_end + 1;
