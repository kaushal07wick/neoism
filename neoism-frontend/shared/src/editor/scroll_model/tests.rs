    ...
    fn vertical_overlay_scroll_pixel_delta_passes_through() {
        assert_eq!(
            vertical_overlay_scroll_pixels(
                &ScrollDelta::Pixels { x: 0.0, y: -38.0 },
                22.0
            ),
            -38.0
        );
    ...
    fn commit_cursor_moved_emits_raw_scroll_then_finish_at_edge() {
        ...
        let TerminalScrollFollowup::BlockMoveRequiresRawResult { moved } =
            commit.followup
        else {
            panic!(
    ...
    fn editor_key_esc_clears_search_highlight_when_editor_present() {
        let plan =
            EditorKeyDispatchPlan::classify("<Esc>", key_ctx(EditorModeClass::Normal));
