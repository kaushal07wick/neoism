    ...
    fn buffer_tabs_scroll_dx_wheel_down_advances_forward() {
        ...
        let dx =
            buffer_tabs_scroll_dx(SessionScrollDelta::Lines { x: 0.0, y: 1.0 }, 0.01);
    ...
    fn buffer_tabs_scroll_dx_pixel_picks_dominant_axis() {
        ...
        let dx =
            buffer_tabs_scroll_dx(SessionScrollDelta::Pixels { x: 2.0, y: 10.0 }, 0.01);
    ...
    fn buffer_tabs_scroll_dx_below_epsilon_returns_zero() {
        let dx =
            buffer_tabs_scroll_dx(SessionScrollDelta::Pixels { x: 0.0, y: 0.001 }, 0.01);
        assert_eq!(dx, 0.0);
    }
    ...
    fn rebase_tab_indexed_btreemap_for_move_matches_hashmap_variant() {
        let mut map: BTreeMap<usize, &'static str> =
            [(0, "a"), (1, "b"), (2, "c"), (3, "d")]
                .into_iter()
                .collect();
    ...
    fn session_layout_active_route_set_returns_none_if_any_leaf_lacks_external_id() {
        let mut layout =
            SessionLayout::new(SessionLeafSpec::new(SessionLeafKind::Terminal));
        layout
            .split_focused(
