    ...
    #[test]
    fn editor_scroll_edge_rows_update_only_when_scroll_or_rows_change() {
        assert!(!editor_scroll_edge_rows_need_update(false, 0, false, false));
        assert!(editor_scroll_edge_rows_need_update(false, 0, false, true));
        assert!(editor_scroll_edge_rows_need_update(false, 0, true, false));
        assert!(editor_scroll_edge_rows_need_update(false, 1, false, false));
        assert!(editor_scroll_edge_rows_need_update(true, 0, false, false));
    }

    #[test]
    fn editor_consume_pending_grid_scroll_animation_dedupes_same_sign() {
        assert_eq!(editor_consume_pending_grid_scroll_animation(2, 1), (1, 0));
        assert_eq!(
            editor_consume_pending_grid_scroll_animation(-2, -1),
            (-1, 0)
        );
    }

    #[test]
    fn editor_consume_pending_grid_scroll_animation_dedupes_opposite_sign() {
        assert_eq!(editor_consume_pending_grid_scroll_animation(-1, 1), (0, 0));
        assert_eq!(editor_consume_pending_grid_scroll_animation(2, -3), (0, -1));
    }
    ...
    #[test]
    fn editor_scroll_frame_plan_keeps_rows_on_origin_shift_canceled_by_source_offset() {
        let previous = EditorScrollGridRenderState {
            source_line_offset: -12,
            pixel_offset_y: -4.0,
            scrollback_origin: Some(100),
        };
        let plan = editor_scroll_frame_plan(
            Some(previous),
            EditorScrollRenderOffset {
                source_line_offset: -13,
                pixel_offset_y: -4.0,
        ...
        assert_eq!(plan.current_source_base, 88);
        assert_eq!(plan.previous_source_base, Some(88));
        assert!(!plan.source_changed);
        assert!(!plan.pixel_changed);
        assert!(plan.scrollback_origin_changed);
        assert!(plan.origin_jump_without_source_shift);
        assert!(!plan.force_full);
        assert_eq!(plan.source_plan, EditorScrollSourcePlan::None);
    }

    #[test]
    fn editor_scroll_frame_plan_forces_full_on_multi_line_jump() {
        let previous = EditorScrollGridRenderState {
            source_line_offset: 0,
            pixel_offset_y: 0.0,
            scrollback_origin: Some(0),
        };
        let plan = editor_scroll_frame_plan(
            Some(previous),
            EditorScrollRenderOffset {
                source_line_offset: 3,
                pixel_offset_y: 0.0,
            },
            Some(0),
            30,
    ...
    #[test]
    fn editor_scroll_frame_plan_uses_shift_for_small_source_move() {
        let previous = EditorScrollGridRenderState {
            source_line_offset: 0,
            pixel_offset_y: 0.0,
            scrollback_origin: Some(0),
    ...
    #[test]
    fn editor_visible_row_sample_prefers_current_visible_rows() {
        assert_eq!(
            editor_visible_row_sample(3, 2, 10, true, 2, 2),
            EditorVisibleRowSample {
                source_row: 5,
                source: EditorVisibleRowSource::Current(5),
            }
        );
    }

    #[test]
    fn editor_visible_row_sample_prefers_edge_rows_before_scrollback_window() {
        assert_eq!(
            editor_visible_row_sample(0, -1, 10, true, 2, 2),
            EditorVisibleRowSample {
                source_row: -1,
                source: EditorVisibleRowSource::AboveEdge(1),
            }
        );
        assert_eq!(
            editor_visible_row_sample(9, 1, 10, true, 2, 2),
            EditorVisibleRowSample {
                source_row: 10,
                source: EditorVisibleRowSource::BelowEdge(0),
            }
        );
    }

    #[test]
    fn editor_visible_row_sample_uses_edge_rows_without_scrollback_grid() {
        assert_eq!(
            editor_visible_row_sample(0, -1, 10, false, 2, 2),
            EditorVisibleRowSample {
                source_row: -1,
                source: EditorVisibleRowSource::AboveEdge(1),
            }
        ...
        assert_eq!(
            editor_visible_row_sample(9, 1, 10, false, 2, 2),
            EditorVisibleRowSample {
                source_row: 10,
                source: EditorVisibleRowSource::BelowEdge(0),
            }
    ...
    #[test]
    fn editor_visible_row_sample_reports_missing_when_no_source_covers_row() {
        assert_eq!(
            editor_visible_row_sample(0, -3, 10, false, 2, 2).source,
            EditorVisibleRowSource::Missing
        );
        assert_eq!(
            editor_visible_row_sample(9, 3, 10, false, 2, 2).source,
            EditorVisibleRowSource::Missing
        );
    }

    #[test]
    fn editor_visible_row_sample_supports_fractional_scroll_edge_output_rows() {
        assert_eq!(
            editor_visible_row_sample(10, -1, 10, false, 2, 2),
            EditorVisibleRowSample {
                source_row: 9,
                source: EditorVisibleRowSource::Current(9),
            }
        );
        assert_eq!(
            editor_visible_row_sample(-1, 1, 10, false, 2, 2),
            EditorVisibleRowSample {
                source_row: 0,
                source: EditorVisibleRowSource::Current(0),
            }
        );
    }

    #[test]
    fn editor_visible_row_sample_falls_back_to_scrollback_when_edge_rows_missing() {
        assert_eq!(
            editor_visible_row_sample(0, -1, 10, true, 0, 0),
            EditorVisibleRowSample {
                source_row: -1,
                source: EditorVisibleRowSource::Scrollback(9),
            }
        );
        assert_eq!(
            editor_visible_row_sample(9, 1, 10, true, 0, 0),
            EditorVisibleRowSample {
                source_row: 10,
                source: EditorVisibleRowSource::Scrollback(0),
            }
        );
    }

    #[test]
    fn editor_grid_hit_cell_inverts_pixel_scroll_transform() {
        let hit = editor_grid_hit_cell(
            18.0,
            29.0,
        ...
        assert_eq!(hit, Some(EditorGridHitCell { row: 3, col: 1 }));
    }

    #[test]
    fn editor_grid_hit_cell_rejects_pointer_over_edge_row_outside_source() {
        let hit = editor_grid_hit_cell(
            18.0,
            21.0,
            [10.0, 20.0, 80.0, 160.0],
    ...
    #[test]
    fn editor_scroll_grid_render_state_reports_source_base() {
        let state = EditorScrollGridRenderState::new(
            EditorScrollRenderOffset {
                source_line_offset: -3,
                pixel_offset_y: 7.0,
            },
        ...
        assert_eq!(state.source_base(), 97);
    }

    // ------------------------------------------------------------
    // trail-cursor + cursorline overlay policy tests
    // ------------------------------------------------------------
    ...
    #[test]
    fn cursorline_plan_target_y_matches_trail_cursor_y() {
        let geom = geom([0.0, 0.0, 800.0, 600.0], 8.0, 10.0, 20.0);
        let scroll = EditorScrollState {
            scroll_position_lines: 0.5,
            elastic_offset_y: 0.0,
            previous_source_line_offset: None,
        };
        let trail = terminal_grid_trail_cursor_destination(TrailCursorPlanInput {
            geometry: geom,
            cursor_row: 7,
            cursor_col: 0,
            visible_rows: 30.0,
        ...
        let line = editor_cursorline_plan(CursorlinePlanInput {
            geometry: geom,
            cursor_row: 7,
            visible_rows: 30.0,
            editor_scroll: scroll,
        });
        // The cursorline target y must equal the trail cursor's
        // destination y so the band and the block glide together.
        assert_eq!(trail.y, line.target_y);
    }

    #[test]
    fn cursorline_plan_pane_width_spans_columns_not_panel_inset() {
        let g = GridPanelGeometry {
            panel_rect: [0.0, 0.0, 800.0, 600.0],
            scaled_margin: ScaledMargin {
                top: 0.0,
                left: 4.0,
                right: 60.0, // chrome scrollbar zone
                bottom: 0.0,
            },
            cell_width: 10.0,
            cell_height: 20.0,
        ...
        // pane_w must be cols * cell_w (full editable band), NOT
        // panel_rect[2] - margins (which would exclude the scrollbar
        // chrome).
        assert_eq!(line.pane_w, 400.0);
        assert_eq!(line.pane_x, 4.0);
    }

    #[test]
    fn cursorline_plan_reports_animating_when_spring_residual_non_zero() {
        let g = geom([0.0, 0.0, 800.0, 600.0], 0.0, 10.0, 20.0);
        // source_line_offset == 0, but pixel_offset_y != 0.
        let line = editor_cursorline_plan(CursorlinePlanInput {
            geometry: g,
            cursor_row: 2,
            visible_rows: 30.0,
            editor_scroll: EditorScrollState {
                scroll_position_lines: 0.2,
                elastic_offset_y: 0.0,
                previous_source_line_offset: None,
            },
        });
        assert!(line.editor_scroll_animating);
        // source_line_offset != 0, pixel_offset_y == 0.
        let line = editor_cursorline_plan(CursorlinePlanInput {
            geometry: g,
            cursor_row: 2,
            visible_rows: 30.0,
            editor_scroll: EditorScrollState {
                scroll_position_lines: 1.0,
                elastic_offset_y: 0.0,
                previous_source_line_offset: None,
            },
        });
        assert!(line.editor_scroll_animating);
        // Both zero -> not animating.
        let line = editor_cursorline_plan(CursorlinePlanInput {
            geometry: g,
            cursor_row: 2,
            visible_rows: 30.0,
    ...
    #[test]
    fn cursorline_plan_cell_height_floored_to_one() {
        // Defensive: even if a caller passes a degenerate cell, the
        // returned cell_height stays whatever the geometry says — the
        // caller (chrome_policy or screen) is responsible for the
        // .round().max(1.0) clamp before constructing the geometry.
        let g = GridPanelGeometry {
            panel_rect: [0.0, 0.0, 800.0, 600.0],
        ...
        assert_eq!(line.cell_height, 1.0);
    }

    #[test]
    fn cursorline_plan_clamps_to_visible_editor_rows() {
        let g = GridPanelGeometry {
            panel_rect: [0.0, 0.0, 800.0, 600.0],
            scaled_margin: ScaledMargin {
                top: 12.0,
        ...
        assert_eq!(line.target_y, 12.0 + 4.0 * 20.0);
    }
