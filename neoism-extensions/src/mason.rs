...
fn mason_asset_to_github_assets(
    ...
    match &asset.target {
        MasonTargetField::Single(t) => vec![GithubAsset {
            target: t.clone(),
            file: asset.file.primary().to_string(),
            bin: bin.clone(),
        }],
    ...
    let install = match kind {
        PurlKind::Github => {
            ...
            let assets: Vec<GithubAsset> = asset_field
                ...
                .into_vec()
                .iter()
                .flat_map(|asset| mason_asset_to_github_assets(asset, &bin_name))
                .collect();
            if assets.is_empty() {
...
mod tests {
    ...
    fn translates_npm_source() {
        let pkg = single_pkg(
            r#"{
                ...
                "categories": ["LSP"],
                "languages": ["Python"],
                "source": { "id": "pkg:npm/pyright@1.1.409" },
    ...
    #[test]
    fn unsupported_purl_returns_err() {
        let pkg = single_pkg(
            r#"{
                "name": "some-gem",
                "description": "ruby thing",
