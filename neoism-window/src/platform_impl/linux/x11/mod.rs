...
pub struct ActiveEventLoop {
    ...
    /// `request_redraw` as a wake marker and consumed by `has_pending`.
    /// The durable redraw state lives on each window's `redraw_pending`.
    ...
    /// X11 Present extension info, when supported by the server.
    /// Currently unused — kept as scaffolding for the eventual switch
    /// from the calloop periodic timer to real `PresentCompleteNotify`
    /// vsync events. See `arm_present_notify`.
    #[allow(dead_code)]
    pub(super) present_ext: Option<x11rb::x11_utils::ExtensionInformation>,
}
...
pub(crate) struct EventLoopState {
    ...
    /// Set by `request_redraw` (via the shared `Arc<AtomicBool>`) and
    /// consumed by `has_pending`. This is only a wake marker; keeping it
    /// set until the next vsync would force zero-timeout polling between
    /// frames and hurt high-refresh pacing.
...
impl<T: 'static> EventLoop<T> {
    pub(crate) fn new(xconn: Arc<XConnection>) -> EventLoop<T> {
        ...
        // Optional: the X11 Present extension. When the server has it
        // (every modern X server does), we replace the drift-prone
        // calloop periodic vsync timer with `PresentCompleteNotify`
        // events delivered exactly on display vblank — same source
        // ghostty/zed use to lock to real refresh on X11. When it's
        // missing (very old / minimal X servers, Xvnc without Present)
        // we fall through to the timer path; correctness is unchanged,
        // just less precise pacing.
        let mut present_ext = xconn
            .xcb_connection()
            .extension_information(x11rb::protocol::present::X11_EXTENSION_NAME)
            .ok()
            .flatten();
        if present_ext.is_some() {
            // Query the version up-front so the server actually
            // initializes the extension state. Failing this query
            // (e.g. on a very old server claiming the extension) drops
            // us back to the timer path.
            ...
            {
                tracing::warn!(
                    "Present extension advertised but query_version failed; falling back to periodic vsync timer"
                );
                present_ext = None;
            }
        }
        ...
        F: FnMut(Event<T>, &RootAEL),
    {
        ...
        wt.x_connection().sync_with_server().map_err(|x_err| {
            EventLoopError::Os(os_error!(OsError::XError(Arc::new(X11Error::Xlib(
                x_err
            )))))
        })?;
        ...
        F: FnMut(Event<T>, &RootAEL),
    {
        ...
        // Drain Present extension special-event queues. These events
        // arrive on the same X11 fd that just woke us, but live on a
        // libxcb special queue (so xlib's drain above never sees
        // them). Each `CompleteNotifyEvent` translates to one vsync
        // hint into `vsync_sender` and a re-arm of `notify_msc`.
        EventProcessor::window_target(&self.event_processor.target)
            .drain_present_events();
...
impl ActiveEventLoop {
    ...
    /// Re-evaluate whether `window_id`'s vsync source should be
    ...
    ///
    /// When the X11 Present extension is available we drive vsync
    /// hints from real `PresentCompleteNotify` events rather than a
    /// software timer. The timer path is kept as a fallback for X
    /// servers without Present (Xvnc minimal builds, ancient X.Org).
    pub(crate) fn update_refresh_loop(&self, window_id: WindowId) {
        ...
        // True-vsync path: ride X11 Present events delivered onto a
        // libxcb special event queue. We arm one notify_msc here, the
        // event drain re-arms after each. When the window goes
        // hidden the event chain stops naturally (no re-arm on the
        // last drained event since `is_mapped` is false), and the
        // RAII subscription is freed when the window drops.
        if self.present_ext.is_some() && present::is_available() {
            if is_visible && self.arm_present_notify(&window, window_id) {
                // Tear down any pre-existing periodic timer — a
                // refresh-rate change on an already-visible window
                // could leave one running from the legacy path.
                let mut slot = window.refresh_state.lock().unwrap();
                if let Some(RefreshState::PeriodicRefresh { token, .. }) = slot.take() {
                    self.loop_handle.remove(token);
                }
                return;
            }
            // arm failed (server wedge, OOM, etc.) — fall through to
            // periodic-timer path below so we still get redraws.
        }
    ...
    /// Set up (or refresh) Present-driven vsync for `window`. Lazily
    /// allocates an XID for `present_select_input` on first call,
    /// installs a libxcb special-event queue (so events bypass xlib's
    /// main drain), and arms the next vblank notification. Subsequent
    /// calls just re-arm. Returns `false` if Present isn't usable on
    /// this server, in which case the caller falls back to the
    /// periodic timer.
    fn arm_present_notify(
        &self,
        window: &std::sync::Arc<UnownedWindow>,
        window_id: WindowId,
    ) -> bool {
        if !present::is_available() {
            return false;
        }
        if self.present_ext.is_none() {
            return false;
        }
        use x11rb::connection::Connection as _;
        use x11rb::protocol::present::{
            ConnectionExt as _, EventMask as PresentEventMask,
        };
        let xconn = &self.xconn;
        let xcb = xconn.xcb_connection();

        let mut eid = window
            .present_event_id
            .load(std::sync::atomic::Ordering::Acquire);
        if eid == 0 {
            // First-time setup for this window: allocate eid, install
            // the special event queue first (so any race-arrived event
            // lands on the special queue instead of in xlib's main
            // queue), THEN call select_input + notify_msc.
            let new_eid = match xcb.generate_id() {
                Ok(id) => id,
                Err(_) => return false,
            };
            // SAFETY: `get_raw_xcb_connection` is the same xcb_connection_t
            // that xcb_register_for_special_xge expects.
            let conn_raw = xcb.get_raw_xcb_connection();
            let sub = match present::register(conn_raw, new_eid) {
                Some(s) => s,
                None => return false,
            };
            if xcb
            ...
            // Stash the subscription on the window so it lives as long
            // as the window. Drop is RAII — releases the special queue
            // on the libxcb side.
            *window.present_subscription.lock().unwrap() = Some(sub);
            window
                .present_event_id
                .store(new_eid, std::sync::atomic::Ordering::Release);
            eid = new_eid;
        }
        let _ = eid; // (kept for future serial-tagging filters)

        let serial = window
            .present_serial
            .fetch_add(1, std::sync::atomic::Ordering::AcqRel);
        // target_msc=0 + divisor=1 + remainder=0 means "fire on the
        // very next vblank". Re-arm in the event handler keeps the
        // stream going.
        if xcb
            .present_notify_msc(window_id.0 as xproto::Window, serial, 0, 1, 0)
            .is_err()
        {
            return false;
        }
        // Flush so the request hits the wire before we park the loop;
        // otherwise the kick-off can sit in xcb's send buffer until
        // the next round-trip and we lose a vblank.
        let _ = xcb.flush();
        true
    }

    /// Drain queued Present events from each visible window's special
    /// event queue. Called from the calloop X11 fd readiness handler
    /// — the same fd readability that signals new X events also
    /// signals new Present events on the special queues.
    pub(crate) fn drain_present_events(&self) {
        if !present::is_available() || self.present_ext.is_none() {
            return;
        }
        let windows: Vec<(WindowId, std::sync::Arc<UnownedWindow>)> = {
            let map = self.windows.borrow();
            map.iter()
                .filter_map(|(id, w)| w.upgrade().map(|w| (*id, w)))
                .collect()
        };
        for (wid, win) in windows {
            // Snapshot the subscription pointer; if the window has
            // none (Present not yet armed, or torn down on hide) we
            // skip it without holding the lock during the poll.
            let sub_ptr: Option<*mut present::XcbSpecialEventT> = {
                let guard = win.present_subscription.lock().unwrap();
                guard.as_ref().map(|s| s.raw())
            };
            let Some(_) = sub_ptr else {
                continue;
            };
            // Drain everything currently queued (the xcb side may
            // have multiple events buffered if we slept past several
            // vblanks).
            loop {
                let window_xid = {
                    let guard = win.present_subscription.lock().unwrap();
                    ...
                    present::poll_complete_notify_window(sub)
                };
                let Some(_window_xid) = window_xid else {
                    break;
                };
                // Pump a vsync hint for this window and re-arm.
                let _ = self.vsync_sender.sender.send(wid);
                self.vsync_sender.waker.ping();
                if win.is_mapped.load(std::sync::atomic::Ordering::Acquire) {
                    self.arm_present_notify(&win, wid);
                }
            }
        }
    }
    ...
    ///
    /// The next deadline is computed as `previous_deadline + rate`,
    /// NOT `Instant::now() + rate`. The latter re-anchors the timer
    /// to wall-clock time on every fire, so any processing delay
    /// inside the callback (or scheduler latency between fire and
    /// dispatch) gets baked into the cadence. On a 165Hz monitor
    /// (rate = 6.06ms) even ~0.5ms of per-cycle drift compounds to
    /// ~140 fps steady-state — that's exactly what neoism's scroll
    /// FPS log was showing (mean_full_ms ≈ 0.5ms but
    /// wait_outside_render_ms ≈ 6.7ms = full vsync cycle + drift).
    /// Anchoring on the previous deadline keeps the timer phase-locked
    /// to the original `Timer::immediate` start point.
    ///
    /// If the loop is behind by more than one period (catch-up after
    /// a stall), clamp to `now` so we don't fire a burst of stale
    /// deadlines all at once.
    fn start_refresh_loop(
        ...
        self.loop_handle
            .insert_source(timer, move |deadline, _, _state| {
                let _ = sender.send(window_id);
                waker.ping();
                let next = deadline + rate;
                let now = Instant::now();
                calloop::timer::TimeoutAction::ToInstant(if next < now {
                    now
                } else {
                    next
                })
...
impl fmt::Display for X11Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            X11Error::NoArgb32Format => f.write_str(
                "winit only supports X11 displays with ARGB32 picture formats",
            ),
        }
    }
}
        ...
        if Device::physical_device(info) {
            for &class_ptr in Device::classes(info) {
                ...
                if ty == ffi::XIValuatorClass {
                    let info =
                        unsafe { &*(class_ptr as *const ffi::XIValuatorClassInfo) };
