...
pub struct UnownedWindow {
    ...
    /// XID allocated for `present_select_input` once the X11 Present
    /// extension is armed for this window. `0` while uninitialized
    /// (or if the extension is unavailable, in which case
    /// `present_subscription` is also `None`).
    pub(crate) present_event_id: std::sync::atomic::AtomicU32,
    /// Monotonic serial supplied to each `present_notify_msc` arm so
    /// future filtering on the returned `serial` field is possible.
    pub(crate) present_serial: std::sync::atomic::AtomicU32,
    /// RAII handle for the libxcb special-event queue receiving this
    /// window's `PresentCompleteNotify` events. `Some` while the
    /// window is mapped and Present is the active vsync source;
    /// `None` otherwise. Drop calls `xcb_unregister_for_special_event`.
    pub(crate) present_subscription:
        std::sync::Mutex<Option<super::present::PresentSubscription>>,
}
...
impl UnownedWindow {
    ...
    pub(crate) fn new(
        ...
        let mut window = UnownedWindow {
            ...
            present_event_id: std::sync::atomic::AtomicU32::new(0),
            present_serial: std::sync::atomic::AtomicU32::new(0),
            present_subscription: std::sync::Mutex::new(None),
        };
