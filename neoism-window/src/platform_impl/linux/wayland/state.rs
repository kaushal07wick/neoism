...
pub struct WinitState {
    ...
    frame_log_started_at: Option<Instant>,
    frame_log_callbacks: u32,
    frame_log_redraw_requested: u32,
    frame_log_present_after_input: u32,
}
...
impl WinitState {
    ...
    fn log_frame_callback(&mut self, redraw_requested: bool, present_after_input: bool) {
        if std::env::var_os("NEOISM_SCROLL_LOG").is_none() {
            return;
        }

        let now = Instant::now();
        let started_at = *self.frame_log_started_at.get_or_insert(now);
        self.frame_log_callbacks = self.frame_log_callbacks.saturating_add(1);
        self.frame_log_redraw_requested += redraw_requested as u32;
        self.frame_log_present_after_input += present_after_input as u32;

        let elapsed = now.duration_since(started_at);
        if elapsed >= Duration::from_millis(500) {
            let fps = self.frame_log_callbacks as f32 / elapsed.as_secs_f32();
            tracing::info!(
                target: "neoism_window::wayland_frame",
                fps,
                callbacks = self.frame_log_callbacks,
                redraw_requested = self.frame_log_redraw_requested,
                present_after_input = self.frame_log_present_after_input,
                "wayland frame callback stats"
            );
            self.frame_log_started_at = Some(now);
            self.frame_log_callbacks = 0;
            self.frame_log_redraw_requested = 0;
            self.frame_log_present_after_input = 0;
        }
    }
}
...
impl WinitState {
    pub fn new(
        ...
        Ok(Self {
            ...
            frame_log_started_at: None,
            frame_log_callbacks: 0,
            frame_log_redraw_requested: 0,
            frame_log_present_after_input: 0,
        })
    }
...
impl CompositorHandler for WinitState {
    ...
    fn frame(
        ...
        let window_id = super::make_wid(surface);
        let window = match self.windows.get_mut().get(&window_id).cloned() {
            Some(window) => window,
            None => return,
        };

        // A frame callback is useful either for an explicit redraw or for
        // the 1-second high-rate input sustain loop. The old gate only
        // woke on `redraw_requested`, which meant compositor callbacks
        // arriving during sustained scroll could be treated as spurious
        // and never reach the `was_received && present_after_input` path
        // in the event loop.
        ...
        let present_after_input = self.should_present_after_input();
        self.log_frame_callback(redraw_requested, present_after_input);
        let will_redraw = redraw_requested || present_after_input;
        if will_redraw {
            self.dispatched_events = true;
        }

        // Do not arm the next callback from inside the callback itself.
        // `pre_present_notify` arms it when the app actually commits a frame;
        // arming here can create a high-rate redraw loop on Hyprland during
        // sustained input even when the compositor has not paced a commit yet.
