...
const FRAME_CALLBACK_STALE_AFTER: Duration = Duration::from_millis(250);

pub(crate) fn frame_callback_throttle_disabled() -> bool {
        ...
        F: FnMut(Event<T>, &RootActiveEventLoop),
    {
        ...
        for window_id in window_ids.iter() {
            let event = self.with_state(|state| {
                ...
                let callback_throttle_disabled = frame_callback_throttle_disabled();
                let was_received = if callback_throttle_disabled {
                    false
                } else {
                    let stale_frame_callback = window.frame_callback_is_stale(
                        Instant::now(),
                        FRAME_CALLBACK_STALE_AFTER,
                    );
                    if window.frame_callback_state() == FrameCallbackState::Requested
                        && !stale_frame_callback
                    {
                        return None;
                    }
                    if stale_frame_callback {
                        tracing::warn!(
                            target: "neoism_window::wayland",
                            ?window_id,
                            stale_after_ms = FRAME_CALLBACK_STALE_AFTER.as_millis(),
                            "recovering stale Wayland frame callback"
                        );
                    }

                    // Reset the frame-callback state so the window can accept a
                    // new callback when the app commits (pre_present_notify).
                    let was_received = matches!(
                        window.frame_callback_state(),
                        FrameCallbackState::Received
