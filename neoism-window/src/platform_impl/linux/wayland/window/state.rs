...
impl WindowState {
    ...
    pub fn frame_callback_is_stale(&self, now: Instant, timeout: Duration) -> bool {
        matches!(self.frame_callback_state, FrameCallbackState::Requested)
            && self
                .frame_callback_requested_at
                .map(|requested_at| now.duration_since(requested_at) >= timeout)
                .unwrap_or(false)
    }
    ...
    /// The frame callback was received and the next callback was armed
    /// immediately. The event loop must still deliver this callback as
    /// `RedrawRequested`, but must keep throttling future redraws on the
    /// newly requested callback.
    pub fn frame_callback_received_and_request_next(&mut self) {
        let surface = self.window.wl_surface();
        self.frame_callback_state = FrameCallbackState::ReceivedAndRequested;
    ...
    pub fn frame_callback_reset(&mut self) {
        match self.frame_callback_state {
            FrameCallbackState::ReceivedAndRequested => {
    ...
    pub fn request_frame_callback(&mut self) {
        ...
        match self.frame_callback_state {
            ...
            FrameCallbackState::Requested | FrameCallbackState::ReceivedAndRequested => {
                ()
            }
        }
    }
...
pub enum FrameCallbackState {
    ...
    Received,
    /// The callback was marked done and the next callback is already in flight.
    ReceivedAndRequested,
}
